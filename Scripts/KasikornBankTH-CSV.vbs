' MT2OFX Input Processing Script Kasikorn Bank (Thailand) CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/KasikornBankTH-CSV.vbs 2     30/06/10 10:30 Colin $"
...
With Params
	...
	.ScriptName = "KasikornBankTH-CSV"
	.FormatName = "Kasikorn Bank (Thailand) CSV"
   ...
   .CodePage = 1252  ' Windows English / Western Europe
	...
	.CurrencyCode = "THB"	' default if not specified in file
...
' to the text between the top-level parentheses.
	...
	.MonthNames = Empty
'Date,Transaction Type,Withdrawal (THB),Deposit (THB),Outstanding Balance (THB),Service Channel
	.Fields = Array( _
		Array(fldBookDate, "Date"), _
		Array(fldSkip, "Transaction Type"), _
		Array(fldAmtDebit, "Withdrawal (THB)"), _
		Array(fldAmtCredit, "Deposit (THB)"), _
		Array(fldClosingBal, "Outstanding Balance (THB)"), _
		Array(fldMemo, "Service Channel") _
	)
...
'MsgBox "In header callback: " & sLine
    If StartsWith(sLine, "Account Number,") Then
        Params.AccountNum = Trim(Mid(sLine, 16))
    End If
    HeaderCallback = True
End Function
