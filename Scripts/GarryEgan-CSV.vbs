' MT2OFX Input Processing Script Garry Egan's CSV format
...
With Params
	...
	.ScriptName = "GarryEgan-CSV"
	.FormatName = "Garry Egan CSV"
	.ParseErrorMessage = "Cannot parse line."
	.ParseErrorTitle = .ScriptName
	.BankCode = "ANZ"
	.AccountNum = "12345678"		' default if not specified in file
	...
	.CurrencyCode = "AUD"	' default if not specified in file
...
' to the text between the top-level parentheses.
	...
	.MonthNames = Empty
' Date	TransactionDetails	Withdrawals	Deposits	Balance
	.Fields = Array( _
		Array(fldBookDate, "Date"), _
		Array(fldMemo, "TransactionDetails"), _
		Array(fldAmtDebit, "Withdrawals"), _
		Array(fldAmtCredit, "Deposits"), _
...
'TO
'FROM
'ATM FOREIGN CURRENCY AMT
'CIRRUS ATM TRANSACTION FEE
'PAY/SALARY FROM
'PAYMENT TO
'TRANSFER <n> TO
'TRANSFER FROM
'NON-AZM ATM TRANSACTIONS - FEE
...
'remove trailing \{\d*\}
'remove trailing " VIC"
'remove trailing "VICAU"
'remove trailing " TAS"
'remove trailing " QLD"
'remove trailing "\d{*}" unless this would empty the payee

Function RemoveTrailing(s, pat)
    re.Pattern = "^(.+)" & pat & "$"
    Set m = re.Execute(s)
	If m.Count = 0 Then
		RemoveTrailing = s
	Else
		RemoveTrailing = m(0).SubMatches(0)
	End If
End Function

Function TransactionCallback(t, vFields)
'MsgBox "In transaction callback: " & t.Memo
    Dim sPayee, sTmp, iTmp
    sTmp = t.Memo
    If StartsWith(sTmp, "ANZ ATM") Then
        t.TxnType = "ATM"
    ElseIf StartsWith(sTmp, "NON-ANZ ATM TRANSACTIONS - FEE") Then
        t.TxnType = "FEE"
    ElseIf StartsWith(sTmp, "NON-ANZ ATM") Then
        t.TxnType = "ATM"
    ElseIf StartsWith(sTmp, "ATM FOREIGN CURRENCY") Then
        t.TxnType = "ATM"
    ElseIf StartsWith(sTmp, "TRANSFER FROM ") Then
        sPayee = Mid(sTmp, 14)
    ElseIf StartsWith(sTmp, "PAY/SALARY FROM ") Then
        sPayee = Mid(sTmp, 16)
    ElseIf StartsWith(sTmp, "EFTPOS ") Then
        t.TxnType = "POS"
        sPayee = Mid(sTmp, 8)
    ElseIf StartsWith(sTmp, "CIRRUS ATM TRANSACTION FEE") Then
        t.TxnType = "FEE"
    ElseIf StartsWith(sTmp, "ACCOUNT SERVICING FEE") Then
        t.TxnType = "FEE"
    ElseIf StartsWith(sTmp, "TO ") Then
        sPayee = Mid(sTmp, 4)
    ElseIf StartsWith(sTmp, "FROM ") Then
        sPayee = Mid(sTmp, 6)
    ElseIf StartsWith(sTmp, "PAYMENT TO ") Then
        sPayee = Mid(sTmp, 12)
    ElseIf StringMatches(sTmp, "TRANSFER (\d+ )?TO ") Then
        iTmp = InStr(sTmp, " TO ")
        sPayee = Mid(sTmp, iTmp + 4)
    Else
        sPayee = sTmp
    End If
    sPayee = Trim(sPayee)
    sPayee = RemoveTrailing(sPayee, "{\d+}")
    sPayee = RemoveTrailing(sPayee, " (VIC|QLD|TAS|VICAU|VIC AU|QLD AU)")
    sTmp = TrimTrailingDigits(sPayee)
    If Len(sTmp) > 0 Then sPayee = sTmp
    t.Payee = Trim(sPayee)
