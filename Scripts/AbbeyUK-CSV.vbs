' MT2OFX Input Processing Script Abbey Bank (UK) CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/AbbeyUK-CSV.vbs 2     25/11/08 22:14 Colin $"

Const ScriptName = "AbbeyUK-CSV"
Const FormatName = "Abbey Bank (UK) Business CSV"
...
aFields = Array( _
	...
	Array(fldBookDate, "Date", "(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[0-3][0-9]"), _
	Array(fldAmount, "Amount"), _
	Array(fldEmpty, "Empty") _
)
...
Function ParseDate(sDate)
	ParseDate = ParseDateEx(CStr(Year(Now())) & "/" & Left(sDate,3) & "/" & Right(sDate, 2), "YMD", "/")
	ParseDate = MostRecent(ParseDate)
End Function
...
' find the payee, transaction type and txn date if we can
			sMemo = Txn.Memo
			If StartsWith(sMemo, "CHEQUE ") Then
				Txn.CheckNum = Trim(Mid(sMemo, 8))
				Txn.TxnType = "CHECK"
			End If
