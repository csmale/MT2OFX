' MT2OFX Input Processing Script Citibank (Belgium) CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/CitibankBE-CSV.vbs 1     20/02/07 11:48 Colin $"

Const ScriptName = "CitibankBE-CSV"
Const FormatName = "Citibank (Belgium) CSV"
...
Dim aFields
aFields = Array( _
	Array(fldBookDate, "Book Date"), _
	Array(fldMemo, "Memo"), _
	Array(fldAmount, "Amount"), _
	Array(fldEmpty, "Nothing"), _
	Array(fldAccountNum, "Account Number", "=ValidateAccount(""%1"")") _
)

' Validate it is a Citibank (Belgium) account number
Private Function ValidateAccount(sAcct)
	ValidateAccount = Validate_Belgium(sAcct)
	If ValidateAccount Then
		ValidateAccount = Left(sAcct, 2) = "95"
	End If
	If Not ValidateAccount Then
		If DebugRecognition Then
			MsgBox "'" & sAcct & "' is geen geldig Citibank België rekeningnummer."
		End If
	End If
End Function
...
' CitiBank Belgium - Expand as additional examples become available
			sTmp = Trim(Left(sMemo, 40))
			If sTmp="UITGAANDE FX TRANSFER" Then
				Txn.Payee = Trim(Mid(sMemo, 81, 40))
			ElseIf sTmp="STORTING IN SPECIEN" Then
				Txn.Payee = "Storting in specien"
			ElseIf sTmp="OVERSCHRIJVING TEN UWE GUNSTE VAN" Then
				Txn.Payee = Trim(Mid(sMemo, 41, 40))
			End If
