...
Const ScriptVersion = "$Header: /MT2OFX/MetroBankUK-CSV.vbs 2     12/11/10 0:01 Colin $"
...
With Params
	...
	.ScriptName = "MetroBankUK-CSV"
	.FormatName = "MetroBank UK CSV"
   ...
   .CodePage = 1252  ' Windows English / Western Europe
	...
	.SkipHeaderLines = 1	' number of lines to skip before the transaction data
...
' 30/10/2010 12:19
	...
	.MonthNames = Empty
' Date,Description,Time,Money Out,Money In,Current Balance
	.Fields = Array()
...
'	.MaxFieldsExpected = 1
	.Properties = Array( _
		Array("AcctNum", "Account number", _
			...
			ptString,,"\d{7,8}", "Please enter a valid account number (7-8 digits).") _
		)
...
'	Set .ReadLineCallback = GetRef("ReadLineCallback")
    Set .User1Callback = GetRef("GetPayee")
End With

Dim vFields1
vFields1 = Array( _
		Array(fldBookDate, "Date"), _
		Array(fldSkip, "Description"), _
		Array(fldSkip, "Time"), _
		Array(fldAmtDebit, "Money Out"), _
		Array(fldAmtCredit, "Money In"), _
		Array(fldClosingBal, "Current Balance") _	
	)

' alternative line formats
' ,30/10/2010 12:19,,,,
'  -> probably transaction date/time
' ,MUJI TOTTENHAM COURT ROAD LONDON W1P W1T 1AZ GBR,,,,
'  -> probably merchant -> payee
Dim vFields2
vFields2 = Array( _
		Array(fldSkip, ""), _
		Array(fldUser1, "Description"), _
		Array(fldSkip, ""), _
		Array(fldSkip, ""), _
		Array(fldSkip, ""), _
		Array(fldSkip, "") _	
...
' initialise to format 1
Params.Fields = vFields1
...
Function LoadTextFile()
	...
	If Not Params.FieldDict.Exists(fldAccountNum) Then
		If PropertyExists("AcctNum") Then
			...
			If Len(sAcct) = 0 Then
				Message True, True, "Please set the Account number on the following screen.", Params.ScriptTitle
            Configure
            sAcct = GetProperty("AcctNum")
            If Len(sAcct) = 0 Then
                LoadTextFile = False
...
Function GetPayee(stmt, t, sField)
    If Not IsNumeric(Left(sField, 2)) Then
        t.Payee = sField
    End If
    ConcatMemo sField
End Function
