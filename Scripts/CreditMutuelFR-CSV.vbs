...
Const ScriptVersion = "$Header: /MT2OFX/CreditMutuelFR-CSV.vbs 1     19/10/10 1:07 Colin $"
...
With Params
	...
	.ScriptName = "CreditMutuelFR-CSV"
	.FormatName = "Crédit Mutuel CSV"
...
'Date d'opération;Date de valeur;Débit;Crédit;Libellé;Solde
	.Fields = Array( _
		Array(fldBookDate, "Date d'opération"), _
		Array(fldValueDate, "Date de valeur"), _
...
'MsgBox "In transaction callback: " & t.Memo
    If StartsWith(t.Memo, "VIR DE ") Then
        t.Payee = Trim(Mid(t.Memo, 8))
    ElseIf StartsWith(t.Memo, "PRLV ") Then
        t.Payee = Trim(Mid(t.Memo, 6))
        t.TxnType = "DIRECTDEBIT"
    ElseIf StartsWith(t.Memo, "VIR WEB VERS ") Then
        t.Payee = Trim(Mid(t.Memo, 14))
    ElseIf StartsWith(t.Memo, "F COTIS ") Then
        t.TxnType = "SRVCHG"
    End If
