' MT2OFX Input Processing Script TJSB CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/ThaneJanataSahakariBankIN-CSV.vbs 1     30/01/09 23:35 Colin $"
...
With Params
	...
	.ScriptName = "TJSBIN-CSV"
	.FormatName = "Thane Janata Sahakari Bank (India) CSV"
	.ParseErrorMessage = "Cannot parse line."
	.ParseErrorTitle = .ScriptName
	.BankCode = "TJSBINBB"	' fake!
	...
	.CurrencyCode = "INR"	' default if not specified in file
	.ColumnHeadersPresent = True	' are the column headers in the file?
	.SkipHeaderLines = 7	' number of lines to skip before the transaction data
...
' Txn. Date, Value Date, Narration, Chq./Ref. Number, Withdrawal Amount, Deposit Amount, Closing Balance 
	.Fields = Array( _
		Array(fldTransactionDate, "Txn. Date"), _
		Array(fldValueDate, "Value Date"), _
		Array(fldMemo, "Narration"), _
		Array(fldCheckNum, "Chq./Ref. Number"), _
		Array(fldAmtDebit, "Withdrawal Amount"), _
		Array(fldAmtCredit, "Deposit Amount"), _
		Array(fldClosingBal, "Closing Balance") _
	)
...
'	.MaxFieldsExpected = 1
	...
	Set .IsValidTxnLine = GetRef("IsValidTxnLine")		' GetRef() of function to call on newly-read line containing txn
...
' in the class
Dim dOpeningBalDate, dClosingBalDate: dOpeningBalDate = NODATE: dClosingBalDate = NODATE
Dim dOpeningBalAmt, dClosingBalAmt
Function HeaderCallback(sLine)
'MsgBox "In header callback: " & sLine
	Dim iColon: iColon = InStr(sLine, ":")
	If iColon = 0 Then
		HeaderCallback = True
		Exit Function
	End If
	Dim sKey, sValue
	sKey = Trim(Left(sLine, iColon-1))
	sValue = Trim(Mid(sLine, iColon+1))
	Select Case sKey
	Case "Account Activity for"
		Params.AccountNum = Trim(Mid(sLine, 24))
	Case "Statement for Period"
		dOpeningBalDate = Params.ParseDate(Left(sValue, 10))
		dClosingBalDate = Params.ParseDate(Right(sValue, 10))
	Case "Opening Balance"
		dOpeningBalAmt = Params.ParseAmount(sValue)
	Case "Closing Balance"
		dClosingBalAmt = Params.ParseAmount(sValue)
	End Select
	HeaderCallback = True
End Function
...
Sub StatementCallBack(s)
'MsgBox "In statement callback"
	s.OpeningBalance.Amt = dOpeningBalAmt
	s.OpeningBalance.BalDate = dOpeningBalDate
	s.ClosingBalance.Amt = dClosingBalAmt
