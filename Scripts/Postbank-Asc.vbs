...
Private Const ScriptVersion = "$Header: /MT2OFX/Postbank-Asc.vbs 9     6/12/09 14:19 Colin $"
...
' check decimal point/comma for use with CDbl
Dim sDecSep
If CDbl("1.234") > 2 Then
    sDecSep = ","
Else
    sDecSep = "."
End If
...
'		MsgBox "stuff not found in first line"
		Exit Function
	End If
	If Right(sLine, 3) = "EUR" Then	' Girotel Online
		LinePat = MainPat
		fldAfBij = 9
		fldOmschrijving1 = 10
		fldOmschrijving2 = 11
...
Function LoadTextFile()
	...
	' set up new transaction, and start a new statement if the account # changes
			...
			LastMemo = ""
			If bIsCSV Then
				Txn.Amt = CDbl(Replace(vFields(fldBedrag),".", sDecSep))
			Else
				Txn.Amt = CDbl(vFields(fldBedrag)) / 100.0
			End If
			If vFields(fldAfBij) = "A" Then
				Txn.Amt = -Txn.Amt
			End If
