...
Const ScriptVersion = "$Header: /MT2OFX/WachoviaUS-WCR.vbs 1     26/07/10 21:33 Colin $"
...
With Params
	...
	.ScriptName = "WachoviaUS-WCR"
	.FormatName = "Wachovia WCR (CSV)"
...
' to the text between the top-level parentheses.
	...
	.MonthNames = Empty
' WBFL 2000045445725,TCHC OPERATING,100707,C,00000500000.00,00000000000.00,00000000000.00,00000000000.00,00000812679.25,00001312679.25,000100707025888,0000000000,195,21ST CENTURY HOLDING COMPA REF=2000041223737666
'Account number,Desc,yymmdd,devit/credit,amount,,,,,,,cknumber,,
	.Fields = Array( _
		Array(fldAccountNum, "Account number", "WB(CS|CT|DC|DE|FL|GA|MD|NC|NS|SC|UK|VA) \d+"), _
		Array(fldMemo, "Description"), _
		Array(fldBookDate, "Date", "\d\d(0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[0-1])"), _
		Array(fldSign, "Credit/Debit", "C|D"), _
		Array(fldAmount, "Amount"), _
		Array(fldSkip, "?"), _
		Array(fldSkip, "?"), _
		Array(fldSkip, "?"), _
		Array(fldSkip, "Balance before"), _
		Array(fldClosingBal, "Balance after"), _
		Array(fldSkip, "?"), _
		Array(fldCheckNum, "Check number"), _
		Array(fldSkip, "?"), _
		Array(fldMemo, "Description 2") _
	)
...
'MsgBox "In statement callback"
    If Left(Stmt.Acct, 2) = "WB" Then
        Stmt.Acct = Mid(Stmt.Acct, 6)
    End If
End Sub
