...
Private Const ScriptVersion = "$Header: /MT2OFX/MT2OFX.vbs 39    28/03/11 0:03 Colin $"
...
Public NODATE
Public ScriptName: ScriptName = "MT2OFX"
	...
	Public Property Let MonthNames(x)
		If IsArray(x) Then
			If (UBound(x) Mod 12) <> 11 Then
				Message True, True, "Length of MonthNames array must be a multiple of 12", ScriptName
...
' ParseDate method
    Public Function ParseDate(sDate)
        If IsEmpty(CustomDateCallback) Then
            ...
            If Len(ParseDateError) > 0 Then
                Message False, True, "Error parsing date '" & "': " & ParseDateError, ScriptName
            End If
        Else 
		...
		For i=LBound(vFields) To UBound(vFields)
				...
				If Left(xFields(i-1)(2), 1) = "=" Then
					...
					If Not Eval(sTmp) Then
						If DebugRecognition Then
							Message True, True, "Field " & CStr(i) & ": '" & sField & "' failed custom match '" & xFields(i-1)(2) & "'", ScriptName
						End If
						Exit Function
					End If
				Else
					If Not StringMatches(sField, xFields(i-1)(2)) Then
						If DebugRecognition Then
							Message True, True, "Field " & CStr(i) & ": '" & sField & "' does not match '" & xFields(i-1)(2) & "'", ScriptName
						End If
						Exit Function
					End If
				End If
			Else
				If sField <> xFields(i-1)(1) Then
					If DebugRecognition Then
						Message True, True, "Field " & CStr(i) & " " & sField & ", expecting " & xFields(i-1)(1), ScriptName
					End If
					Exit Function
				End If
			End If
...
' this is going to cause a problem as functions in a script module are not exposed to Global
	...
	' NB: ParseDate will throw an error on an invalid date! need to sort this
			...
			If Not bTmp Then
				If DebugRecognition Then
					Message True, True, "Field " & i & " (" & sField & ") failed to match '" & sPat & "'", ScriptName
				End If
				Exit For
			End If
		Next
...
' 20091013 CS: codepage now in class. only use it if it has been set explicitly
    If p.CodePage <> CP_ACP Then
...
'    Dim Acct   ' account number for detecting statement change
    Dim FirstTxn
' Initialisation and termination
    Private Sub Class_Initialize
...
Private xContexts: Set xContexts=CreateObject("Scripting.Dictionary")
Private st			' current ProcessContext

Public Function FindContext(sKey)
	If xContexts.Exists(sKey) Then
		Set FindContext = xContexts(sKey)
	Else
		Set FindContext = Nothing
	End If
End Function

Public Function GetContext(sKey)
    Dim x
    Set x = FindContext(sKey)
    If x Is Nothing Then
        If xContexts.Count > 0 And Not VersionAtLeast("3.5.37") Then
            Message True, True, "Multiple contexts not supported in versions before 3.5.37.", ScriptName
            Abort
            Dim a: a=xContexts.Items
            Set GetContext = a(0)
            Exit Function
        End If
        Set x = New ProcessContext
        Set xContexts(sKey) = x
...
Public Sub SetContext(sKey)
    Set st = GetContext(sKey)
    If VersionAtLeast("3.5.37") Then
        SetStatement st.Stmt
    End If
'    msgbox "context set to " & skey
End Sub

Public Function CurrentContext()
    Set CurrentContext = st
End Function
'========================================================================
...
' 20090827 CS: auto detect separator
            ...
            If TypeName(vFields) <> "Variant()" Then
                Message True, True, "Parse Error: '" & sLine & "'", vbOkOnly+vbCritical, "Parse Error", p.ScriptName
                Abort
                Exit Function
            End If
            nFields = UBound(vFields) - LBound(vFields) + 1
...
' this is the last point that the context can be switched
            If p.PreParseCallback(vFields) Then

' if no explicit context, get default state
            If st Is Nothing Then SetContext ""

' should we start a new statement here?
            bNewStatement = False
            If IsEmpty(st.Stmt) Then
                bNewStatement = True
'msgbox "New statement due to no current statement"
                sAcct = p.AccountNum
            ElseIf p.FieldDict.Exists(fldAccountNum) Then
                If st.Stmt.Acct <> vFields(p.FieldDict.Item(fldAccountNum)) Then
                    If st.Stmt.Acct = "" Then
'msgbox "Setting account number from file to " & vFields(p.FieldDict.Item(fldAccountNum))
                        st.Stmt.Acct = vFields(p.FieldDict.Item(fldAccountNum))
                    Else
...
'msgbox "New statement due to change in account number present in file"
                    ...
                    End If
                End If
            ElseIf st.Stmt.Acct <> p.AccountNum Then
                If st.Stmt.Acct = "" And p.AccountNum <> "" Then
'msgbox "Setting account num from parameters to " & p.AccountNum
                    st.Stmt.Acct = p.AccountNum
...
' if needed set up a new statement
            If bNewStatement Then
                PostProcessStatement p, st.Stmt
                Set st.Stmt = NewStatement()
                st.Stmt.OpeningBalance.Ccy = p.CurrencyCode
                If Not p.NoAvailableBalance Then st.Stmt.AvailableBalance.Ccy = p.CurrencyCode
                st.Stmt.ClosingBalance.Ccy = p.CurrencyCode
                st.Stmt.BankName = p.BankCode
                st.Stmt.Acct = sAcct
                st.Stmt.BranchName = p.BranchCode
                st.Stmt.AcctType = p.AccountType
                st.FirstTxn = True               
            End If
            ...
            ElseIf tlType = txnlineCONTINUE Then
                If st.FirstTxn Then
                    Message True, True, "Continuation line with no active transaction", p.ScriptName
                    Abort
                End If
            End If
            ...
            For i=LBound(vFields) To UBound(vFields)
                ...
                case fldAccountNum
                    st.Stmt.Acct = sField
                case fldBranch
                    st.Stmt.BranchName = sField
                case fldCurrency
                    sTmp = MapCurrencyToISO4217(sField)
                    If Len(sTmp) > 0 Then sField=sTmp
                    st.Stmt.OpeningBalance.Ccy = sField
                    st.Stmt.ClosingBalance.Ccy = sField
                    If Not p.NoAvailableBalance Then st.Stmt.AvailableBalance.Ccy = sField
                Case fldClosingBal
                    If p.OldestLast Then
                        If st.FirstTxn Then
                            st.Stmt.ClosingBalance.Amt = p.ParseAmount(sField)
                        End If
                    Else
                        st.Stmt.ClosingBalance.Amt = p.ParseAmount(sField)
                    End If
                case fldAvailBal
                    st.Stmt.AvailableBalance.Amt = p.ParseAmount(sField)
                case fldBookDate
                    Txn.BookDate = p.ParseDate(sField)
                    st.da.Process Txn.BookDate
                case fldValueDate
                    Txn.ValueDate = p.ParseDate(sField)
                    st.da.Process Txn.ValueDate
                Case fldTransactionDate
                    Txn.TxnDate = p.ParseDate(sField)
                    Txn.TxnDateValid = (Txn.TxnDate <> NODATE)
                    st.da.Process Txn.TxnDate
                Case fldTransactionTime
                ...
                Case fldBalanceDate
                    ...
                    If dBal > st.Stmt.ClosingBalance.BalDate Or st.Stmt.ClosingBalance.BalDate = NODATE Then
                        st.Stmt.ClosingBalance.BalDate = dBal
                        st.Stmt.AvailableBalance.BalDate = dBal
                    End If
                    If dBal < st.Stmt.OpeningBalance.BalDate Or st.Stmt.OpeningBalance.BalDate = NODATE Then
                        st.Stmt.OpeningBalance.BalDate = dBal
                    End If
                Case fldPayee
...
' 20080827 CS: add user fields
                Case fldUser1, fldUser2, fldUser3, fldUser4, fldUser5
                    p.DoUserFieldCallback iField, st.Stmt, Txn, sField
...
' keep tabs on the statement/balance Date
            st.Stmt.ClosingBalance.BalDate = st.da.MaxDate
            st.Stmt.OpeningBalance.BalDate = st.da.MinDate
...
' postprocess the final statements
    For Each sTmp in xContexts.Keys
        PostProcessStatement p, xContexts(sTmp).Stmt
    Next
...
End Function
Public Function MapCurrencyToISO4217(sCurr)
	MapCurrencyToISO4217 = GetConfigStringEx("ToISO4217", sCurr, "", Cfg.AppPath & "\currencies.ini")
End Function

'
' Extra utility functions for custom output formats
'
Public Function FixedString(sIn, nChars)
    If Len(sIn) = nChars Then
        FixedString = sIn
    ElseIf Len(sIn) > nChars Then
        FixedString = Left(sIn, nChars)
    Else
        FixedString = sIn & Space(nChars - Len(sIn))
    End If
End Function
Public Function FixedInteger(iIn, nChars)
    Dim sTmp
    sTmp = CStr(CInt(iIn))
    If Len(sTmp) = nChars Then
        FixedInteger = sIn
    ElseIf Len(sTmp) > nChars Then
        FixedInteger = Right(sTmp, nChars)
    Else
        FixedString = String("0", nChars - Len(sTmp)) & sTmp
    End If    
End Function
Public Function FixedAmount(dAmt, nDecimals, sDec)

End Function

Class CustomOutputParameters
    Dim Fields
    
End Class

Function GetAccountNumber(sAccountProp, sMultiAccountProp, sPreviousProp, sTitle, sHelp, sValidate)
    Dim sAcct, sList, sDefault, bFixedList, bMultiAccount, sNewList
    
    sList = GetProperty(sAccountProp)
    If InStr(sList, "\") = 0 Then ' no list present - single entry or empty
        If Len(sMultiAccountProp) = 0 Then
            bMultiAccount = False
        Else
            bMultiAccount = CBool(GetProperty(sMultiAccountProp))
        End If
        If Len(sList) > 0 And Not bMultiAccount Then ' single entry given and this is all we need
            GetAccountNumber = sList
            Exit Function
        End If
            ' otherwise show the choice dialog
    End If
    bFixedList = False
    sDefault = GetProperty(sPreviousProp)
    sAcct = ChooseFromList(sList, sDefault, sTitle, sHelp, bFixedList, sNewList, "\", sValidate)
    If Len(sAcct) > 0 Then
        ' save properties
        SetProperty sAccountProp, sNewList
        SetProperty sPreviousProp, sAcct
    Else
        Abort
    End If
    GetAccountNumber = sAcct
End Function
