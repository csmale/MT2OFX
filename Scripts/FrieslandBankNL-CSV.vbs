' MT2OFX Input Processing Script Friesland Bank CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/FrieslandBankNL-CSV.vbs 2     20/04/08 10:02 Colin $"

Const ScriptName = "FrieslandBankNL-CSV"
Const FormatName = "Friesland Bank CSV-formaat"
Const ParseErrorMessage = "Kan informatie niet ontleden."
Dim ParseErrorTitle : ParseErrorTitle = ScriptName

Const DebugRecognition = False	' enables debug code in recognition
Const BankCode = "FRBKNL2L"
...
'"Boekdatum","Valutadatum","Rek.nr.klant","Muntsoort","Bedrag","Bij / Af","Transactiesoort","Tegenrekening",
'  "Naam (tegenrekening)","Adres (tegenrekening)","Postcode Woonplaats (tegenrekening)","Betalingskenmerk","Omschrijving"
Dim aFields
aFields = Array( _
	Array(fldBookDate, "Boekdatum"), _
	Array(fldValueDate, "Valutadatum"), _
	Array(fldAccountNum, "Rek.nr.klant"), _
	Array(fldCurrency, "Muntsoort"), _
	Array(fldAmount, "Bedrag"), _
	Array(fldSkip, "Bij / Af"), _
	Array(fldSkip, "Transactiesoort"), _
	Array(fldSkip, "Tegenrekening"), _
	Array(fldPayee, "Naam (tegenrekening)"), _
	Array(fldSkip, "Adres (tegenrekening)"), _
	Array(fldSkip, "Postcode Woonplaats (tegenrekening)"), _
	Array(fldMemo, "Betalingskenmerk"), _
...
Function LoadTextFile()
	...
	' set up new transaction, and start a new statement if the account # changes
			If FieldDict.Exists(fldAccountNum) Then
				If sAcct <> Trim(vFields(FieldDict(fldAccountNum))) Then
		...
		' this initialisation should be in the class constructor!! (fixed in 3.3.5)
			...
			For i=1 To UBound(vFields)
				...
				Case fldChequeNum
					Txn.CheckNum = sField
				case fldMemo
' FrieslandBank - Betalingskenmerk contains "-" if it is unknown or n/a
