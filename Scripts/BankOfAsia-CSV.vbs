' MT2OFX Input Processing Script Bank of Asia #-separated format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/BankOfAsia-CSV.vbs 6     21/06/09 21:26 Colin $"

Const ScriptName = "BankOfAsia-CSV"
Const FormatName = "Bank of Asia #-separated"
...
Const CurrencyCode = "THB"	' default if not specified in file
...
Dim FileRecognised			' been through recognition process
...
' For banking account:
' Date      #Decription                                                  #Cheque Num#Withdrawal                    #Deposit                       #Balace                        #Teller ID #
' For saving account:
' Date      #Decription                                                  #Withdrawal                    #Deposit                       #Balace                        #Teller ID #
...
aFieldsSav = Array( _
	...
	Array(fldMemo, "Decription"), _
	Array(fldAmtDebit, "Withdrawal"), _
	Array(fldAmtCredit, "Deposit"), _
	Array(fldClosingBal, "Balace"), _
	Array(fldMemo, "Teller ID"), _
	Array(fldSkip, "") _
)
Dim aFieldsCur
aFieldsCur = Array( _
	Array(fldBookDate, "Date"), _
	Array(fldMemo, "Decription"), _
	Array(fldChequeNum, "Cheque Num"), _
	Array(fldAmtDebit, "Withdrawal"), _
	Array(fldAmtCredit, "Deposit"), _
	Array(fldClosingBal, "Balace"), _
	Array(fldMemo, "Teller ID"), _
...
aPropertyList = Array( _
	Array("BoAAccountCur", "Account Number (Current a/c)", _
		"Current Account number for Bank of Asia", _
		ptString), _
	Array("BoAAccountSav", "Account Number (Savings a/c)", _
		"Savings Account number for Bank of Asia", _
		ptString) _
	)

Sub Initialise()
    LogProgress ScriptVersion, "Initialise"
    FileRecognised = False
...
Function RecogniseTextFile()
	...
	If TypeName(vFields) <> "Variant()" Then
		...
		Exit Function
	End If
' Bank of Asia - check for Savings Account vs. Current Account
	If UBound(vFields) > 2 Then
		If Trim(vFields(3)) = "Cheque Num" Then
			aFields = aFieldsCur
			AccountNum = GetProperty("BoAAccountCur")
			If DebugRecognition Then
				MsgBox "BoA Current account detected"
			End If
		Elseif Trim(vFields(3)) = "Withdrawal" Then
			aFields = aFieldsSav
			AccountNum = GetProperty("BoAAccountSav")
			If DebugRecognition Then
				MsgBox "BoA Savings account detected"
			End If
		Else
			If DebugRecognition Then
				MsgBox "Cannot determine if it's a current or savings account!"
			End If
			aFields = Array()	' this will simply cause recognition to fail
		End If
		NumFieldsExpected = UBound(aFields)+1
	Else
		If DebugRecognition Then
			MsgBox "Not enough fields for Bank of Asia"
		End If
		Exit Function
	End If
...
Function LoadTextFile()
	...
	LoadTextFile = False
	If Not FileRecognised Then
		If Not RecogniseTextFile() Then
			Message True, True, "Cannot recognise this file!", ScriptName
			Exit Function
		End If
	End If
	...
	Do While Not AtEOF()
		...
		If Len(sLine) > 0 And Left(sLine,1) <> CSVSeparator Then
			...
			End If
			If UBound(vFields) <> NumFieldsExpected Then
				Message True, True, "Wrong number of fields - got " & UBound(vFields) & " instead of " & NumFieldsExpected & " - " & sLine, ScriptName
				Abort
				Exit function
			End If
...
' bank of asia: available balance not available!
			Stmt.AvailableBalance.Ccy = ""
' bank of asia: capture reference data coz includes a payee account!
		Elseif Left(sLine, 11) = "#REFER TO :" Then
			sTmp = Mid(sLine, 12)
			i = InStr(sTmp, "#")
			If i > 0 Then
				sTmp = Left(sTmp, i-1)
			End If
			ConcatMemo Trim(sTmp)
		End If
	Loop
	LoadTextFile = True
End Function
