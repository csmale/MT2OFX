' MT2OFX Input Processing Script Peace Hills Trust (Canada) CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/PeaceHillsCA-CSV.vbs 2     1/11/10 22:22 Colin $"
...
With Params
	...
	.ScriptName = "PeaceHillsCA-CSV"
	.FormatName = "Peace Hills Trust (Canada) CSV"
	...
	.CurrencyCode = "CAD"	' default if not specified in file
...
' to the text between the top-level parentheses.
	...
	.MonthNames = Empty
' "000000125591-044-603  -00001,01-Mar-2010,""Clearing Cheque                                                                       "",55697,250.00,,-29112.65"
	.Fields = Array( _
		Array(fldAccountNum, "Account number", "\d{12}-\d{3}-\d{3}  -\d{5}"), _
...
' MsgBox "In read line callback: " & sLine
    If Left(sLine, 1) = """" And Right(sLine, 1) = """" Then
        sLine = Replace(Mid(sLine, 2, Len(sLine)-2), """""", """")   ' replace double double quotes with single double quotes...
    End If
