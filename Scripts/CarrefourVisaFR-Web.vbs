...
Const ScriptVersion = "$Header: /MT2OFX/carrefourVisaFR-Web.vbs 5     18/09/06 22:08 Colin $"

Const ScriptName = "CarrefourVisaFR-Web"
Const FormatName = "Carrefour Visa France web page clipboard format"
...
Const BankCode = "CARRFRZZ"
Const CSVSeparator = ""
Const TxnLinePattern = "(\d\d/\d\d/\d{2,4})(?: -)? (.*?)  ([-+]?\d[\d ]*.\d\d) (Comptant (?:différé|immédiat)) *"
' 27/02/2006 BALITRAND  37.08 Comptant différé
...
aPropertyList = Array( _
	Array("CarvFrCompteVideOK", "Permettre des numéros compte vides", _
		"Permettre l'utilisation des numéros compte vides. Attention: les fichiers OFX exigent un numéro compte non-vide!", _
		ptBoolean), _
	Array("CarvFrCompteB", "Numéro compte banque", _
		"Le numéro du compte banque pour Carrefour Visa France.", _
		ptString), _
	Array("CarvFrCompteC", "Numéro compte CC", _
		"Le numéro du compte carte crédit pour Carrefour Visa France.", _
		ptString) _
	)
...
' Axa France: detect bank account or credit card
		If AccountNum = "" Then
			...
			If AccountNum = "" And Not GetProperty("CarvFrCompteVideOK") Then
				Exit Function
			End If
		End If
	...
	' set up new transaction, and start a new statement if the account # changes
		...
		' this initialisation should be in the class constructor!! (fixed in 3.3.5)
			...
			Next
' transaction type
' NB: Carrefour doesn't indicate credit/debit in the file. We assume everything is a payment, except certain "magic"
' transactions which we can recognise as a settlement (transfer in from a bank account). Remember this is a credit card!
			If StartsWith(Txn.Memo, "PRELEVEMENT ") Then
				Txn.Amt = -Txn.Amt
			End If
			If Txn.Amt < 0 Then
				Txn.TxnType = "PAYMENT"
...
' Axa France: find the cheque number
			...
			End If
' Carrefour/Visa France: derive the payee from the memo
			If Txn.TxnDateValid Then
				Txn.Payee = Mid(Txn.Memo, 7)
			Elseif StartsWith(Txn.Memo, "RET ESPECE ") Then
				Txn.Payee = Mid(Txn.Memo, 12)
				Txn.TxnType = "ATM"
			Else
				Txn.Payee = Txn.Memo
			End If
...
' for now we have to avoid that construction
		sAcct = GetProperty("CarvFrCompteC")
	...
	End If
	If Len(sAcct) = 0 and Not GetProperty("CarvFrCompteVideOK") Then
		sAcct = InputBox(sPrompt, _
			"Carrefour/Visa - Compte inconnu", _
			sAcct)
		If Len(sAcct) = 0 Then
...
'		SetProperty sProp, sAcct
		If bIsCC Then
			SetProperty "CarvFrCompteC", sAcct
		Else
			SetProperty "CarvFrCompteB", sAcct
		End If
		SaveProperties ScriptName, aPropertyList
	End If
	GetAccountNum = sAcct
