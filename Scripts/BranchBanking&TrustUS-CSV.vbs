' MT2OFX Input Processing Script BB&T CSV (TXT) format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/BranchBanking&TrustUS-CSV.vbs 1     28/03/11 0:13 Colin $"
...
With Params
	...
	.ScriptName = "BranchBanking&TrustUS-CSV"
	.FormatName = "Branch Banking & Trust CSV"
...
'MsgBox "In transaction callback: " & t.Memo
    Dim sTmp, aMemo, iTmp
    Select Case vFields(2)
    Case "POS"
        t.TxnType = "POS"
        aMemo = ParseLineFixed(t.Memo, "(.*) \d\d-\d\d(-\d\d)? \d\d\d\d .*")
        If UBound(aMemo) >= 1 Then
            t.Payee = aMemo(1)
        End If
    Case "Debit"
        If StartsWith(t.Memo, "ONLINE PMT ") Then
            sTmp = Mid(t.Memo, 11)
            iTmp = InStr(sTmp, "CKF*")
            If iTmp > 0 Then
                t.Payee = Trim(Left(sTmp, iTmp-1))
            End If
        End If
    End Select
