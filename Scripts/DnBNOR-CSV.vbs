...
Const ScriptVersion = "$Header: /MT2OFX/DnBNOR-CSV.vbs 4     25/04/06 21:39 Colin $"

Const ScriptName = "DnBNOR-CSV"
Const FormatName = "DnBNOR CSV Format"
...
Const CurrencyCode = "NOK"	' default if not specified in file
...
aFields = Array( _
	...
	Array(fldMemo, "Forklaring"), _
	Array(fldValueDate, "Rentedato"), _
	Array(fldAmtDebit, "Ut fra konto"), _
	Array(fldAmtCredit, "Inn på konto") _
)
...
aPropertyList = Array( _
	Array("AcctNum", "Account Number", _
		"The account number for DnBNOR", _
		ptString) _
	)
...
Function LoadTextFile()
	...
	LoadTextFile = False
	AccountNum = GetProperty("AcctNum")
	If AccountNum = "" Then
		Message True, True, "Please set the account number in the script properties first!", ScriptName
		Exit Function
	End If
...
' find the payee, transaction type and txn date if we can
			...
			End If
' DnBNOR
			Txn.Memo = Replace(Txn.Memo, "&amp;", "&")
			If StartsWith(Txn.Memo, "Varekjøp ") Then
				iTmp = InStr(Txn.Memo, " Aut. ")
				If iTmp > 0 Then
					Txn.Payee = Trim(Mid(Txn.Memo, iTmp+12))
					Txn.TxnType = "POS"
				End If
			Elseif StartsWith(Txn.Memo, "Ubetjent Varekjøp ") Then
				Txn.Payee = Mid(Txn.Memo, 102)
				Txn.TxnType = "POS"
			Elseif StartsWith(Txn.Memo, "Lønn ") Then
				iTmp = InStr(Txn.Memo, ",")
				If iTmp>0 Then
					Txn.Payee = Trim(Mid(Txn.Memo, iTmp+1))
				End If
			Elseif StartsWith(Txn.Memo, "Pensjon Eller Trygd ") Then
				Txn.Payee = Mid(Txn.Memo, 37)			
			Elseif StartsWith(Txn.Memo, "Visa ") Then
				iTmp = InStr(Txn.Memo, ",")
				If iTmp>0 Then
					Txn.Payee = Trim(Mid(Txn.Memo, iTmp+1))
				Else
					Txn.Payee = Mid(Txn.Memo, 37)
				End If
				If StringMatches(Txn.Payee, "[A-Z][a-z][a-z] \d+,\d\d .*") Then
					iTmp = InStr(5, Txn.Payee, " ")
					If iTmp > 0 Then
						Txn.Payee = Trim(Mid(Txn.Payee, iTmp+1))
					End If
				End If
			Elseif StartsWith(Txn.Memo, "Visa-kostnad ") Then
				Txn.Payee = Mid(Txn.Memo, 37)
				Txn.TxnType = "SRVCHG"
			Elseif StartsWith(Txn.Memo, "Transaksjonskostnad ") Then
				Txn.Payee = "Transaksjonskostnad"
				Txn.TxnType = "SRVCHG"
			Elseif StartsWith(Txn.Memo, "Omkostninger ") Then
				Txn.Payee = Mid(Txn.Memo, 37)
				Txn.TxnType = "SRVCHG"
			Elseif StartsWith(Txn.Memo, "Uttak i Automat ") Then
				Txn.Payee = "Uttak i Automat"			
				Txn.TxnType = "ATM"
			Elseif StartsWith(Txn.Memo, "Overføring Innland ") Then
				Txn.Payee = Mid(Txn.Memo, 37)
				Txn.Payee = Replace(Txn.Memo, " Nettgiro M/meld", "")
				Txn.Payee = Replace(Txn.Memo, " Nettgiro M/kid", "")
			Elseif StartsWith(Txn.Memo, "Giro ") Then
				Txn.Payee = Mid(Txn.Memo, 37)
				Txn.Payee = Replace(Txn.Payee, " Nettgiro M/meld", "")
				Txn.Payee = Replace(Txn.Payee, " Nettgiro M/kid", "")
			Elseif Txn.Memo = "Renter" Then
				Txn.Payee = Txn.Memo
				Txn.TxnType = "INT"
			Else
		MsgBox "Memo: '" & Txn.Memo & "'"
			End If
' DnBNOR: set up FITID based on value Date
			If dBal <> Txn.ValueDate Then
				iSeq = 1
				dBal = Txn.ValueDate
			End If
			Txn.FITID = CStr(Year(dBal)) _
                & "." & Right("000" & DatePart("y", dBal), 3) _
...
' DnB NOR: Closing balance date derived from last transaction
			Stmt.ClosingBalance.BalDate = Txn.ValueDate
' DnB NOR: value date on first transaction is the start of the period
			If Stmt.OpeningBalance.BalDate = NODATE Then
				Stmt.OpeningBalance.BalDate = Txn.ValueDate
			End If
' DnB NOR: no available balance
