...
Const ScriptName = "DBFFamEngl-CSV"
Const FormatName = "Deutsche Bank Frankfurt English CSV format"
...
aFields = Array( _
        ...
        Array(fldMemo, "Transactions Payment details"), _
...
' Transactions - persönliches Konto (00) ;;;Customer number: 100/3910000;;,,,
                j = InStr(sLine, ";Customer number: ")
                If j > 0 Then
                        AccountNum = Left(Mid(sLine, j + 18), 11)
                End If
        Next
        If ColumnHeadersPresent Then
                sLine = ReadLine()
...
' Deutsche Bank: break on balance line
                        If StartsWith(vFields(1), "Account balance") Then
                                Exit Do
                        End If
...
' Deutsche Bank: we should be on the balance line Now
        If StartsWith(vFields(1), "Account balance ") Then
                Stmt.ClosingBalance.BalDate = ParseDate(Mid(vFields(1), 18, 10))
