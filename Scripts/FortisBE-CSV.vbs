...
Const ScriptVersion = "$Header: /MT2OFX/FortisBE-CSV.vbs 2     20/04/08 10:02 Colin $"

Const ScriptName = "FortisBE-CSV"
Const FormatName = "Fortis (Belgium) CSV Format"
...
Dim aFields
aFields = Array( _
	Array(fldFITID, "JAAR + REFERTE"), _
	Array(fldBookDate, "UITVOERINGSDATUM"), _
	Array(fldValueDate, "VALUTADATUM"), _
	Array(fldAmount, "BEDRAG"), _
	Array(fldCurrency, "MUNT V/D REKENING"), _
	Array(fldPayee, "TEGENPARTIJ VAN DE VERRICHTING"), _
...
Function LoadTextFile()
	...
	Dim dMinDate
	Dim FirstTxn
	Dim r: Dim m: Dim sPat	' for regexp work
...
' tidy up the memo
			sPat=""
			If Txn.Payee = "DOMICILIERING" Then
				sPat = ".*\w\d+,\d\d[+-](.*) MEDEDELING :.*"
				Txn.TxnType = "REPEATPMT"
			ElseIf Txn.Payee = "BETALING BANCONTACT/MISTER CASH" Then
				sPat = ".*\w\d+,\d\d[+-](.*)  .* KAART .*"
				Txn.TxnType = "POS"
			ElseIf Txn.Payee = "OPVRAGING BANCONTACT/MISTER CASH" Then
				Txn.Payee = "KASOPNAME"
				Txn.TxnType = "ATM"
			ElseIf StartsWith(Txn.Memo, "OVERSCHRIJVING VAN REK") Then
				sPat = ".*\w\d+,\d\d[+-]VAN (.*) MEDEDELING :.*"
			ElseIf StartsWith(Txn.Memo, "OVERSCHRIJVING OP REK.") Then
				sPat = ".*\w\d+,\d\d[+-]VAN (.*) MEDEDELING :.*"
			End If
			If Len(sPat) > 0 Then
				r.Pattern = sPat
				Set m = r.Execute(Txn.Memo)
				If Not m Is Nothing Then
					If m.Count > 0 Then
						Txn.Payee = Replace(m(0).SubMatches(0), " VIA PC BANKING", "")
					End If
				End If
			End If
