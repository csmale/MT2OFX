...
Const ScriptVersion = "$Header: /MT2OFX/mBankVisaPL-CSV.vbs 3     1/06/09 23:42 Colin $"
...
With Params
	...
	.ScriptName = "mBankVisaPL-CSV"
	.FormatName = "mBank (Poland) Visa Credit Card CSV"
	...
	.CurrencyCode = "PLN"	' default if not specified in file
	.ColumnHeadersPresent = True	' are the column headers in the file?
	.SkipHeaderLines = 32	' number of lines to skip before the transaction data
...
' to the text between the top-level parentheses.
	...
	.MonthNames = Empty
' #Nr oper.;#Data oper.;#Data księg.;#Rodzaj operacji;#Szczegóły operacji;#Kwota w walucie oryginalnej;#Waluta oryginalna;#Kwota w PLN;
	.Fields = Array( _
		Array(fldSkip, "#Nr oper."), _
		Array(fldBookDate, "#Data oper."), _
		Array(fldValueDate, "#Data księg."), _
		Array(fldSkip, "#Rodzaj operacji"), _
		Array(fldMemo, "#Szczegóły operacji"), _
		Array(fldMemo, "#Kwota w walucie oryginalnej"), _
		Array(fldMemo, "#Waluta oryginalna"), _
		Array(fldAmount, "#Kwota w PLN"), _
		Array(fldSkip, "") _
	)
...
Sub Initialise()
	...
	If bPLNPLZ Then
		If Params.CurrencyCode = "PLN" Then
...
' in the class
'#Saldo początkowe;1 095,64;
'#Saldo końcowe;2 030,61;

Dim dClosingBal, dOpeningBal, sAcct
...
'MsgBox "In header callback: " & sLine
	If Left(sLine, 1) = "#" Then
		...
		If iTmp > 0 Then
			sHdr = Left(sLine, iTmp-1)
			sTmp = Mid(sLine, iTmp+1)
			iTmp = InStr(sTmp, Params.CSVSeparator)
			If iTmp > 0 Then
		...
		Case "#Saldo początkowe"
			dOpeningBal = -Params.ParseAmount(sTmp)
		Case "#Saldo końcowe"
...
'MsgBox "In transaction callback: " & t.Memo
	Dim iSp
	If Len(Trim(vFields(5))) > 0 Then	' we have a real transaction description
		iSp = InStr(t.Memo, " ")
		If iSp > 0 Then
			t.Payee = Trim(Mid(t.Memo, iSp+1))
			iSp = InStr(t.Payee, Cfg.MemoDelimiter)
			If iSp > 0 Then
				t.Payee = Left(t.Payee, iSp-1)
			End If
		End If
	End If
...
Function IsValidTxnLine(sLine)
	If StartsWith(sLine, "HISTORIA TRANSAKCJI DLA KARTY") Then
		sAcct = Mid(sLine, 31, 16)
	End If
	IsValidTxnLine = (IsNumeric(Left(Trim(sLine), 1)))
End Function
