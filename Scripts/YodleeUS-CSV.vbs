' MT2OFX Input Processing Script Yodlee CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/YodleeUS-CSV.vbs 2     30/01/11 13:38 Colin $"
...
With Params
	...
	.ScriptName = "Yodlee-CSV"
	.FormatName = "Yodlee CSV Format"
...
' MsgBox "In preparse callback: " & UBound(vFields) & " fields."
    Dim sAcct, sBank, sTmp
    sTmp = vFields(11)
    Dim iSep
    iSep = InStr(sTmp, " - ")
    If iSep > 0 Then
        sBank = Left(sTmp, iSep-1)
        sAcct = Mid(sTmp, iSep+3)
        SetContext sTmp
        Params.BankCode = sBank
