' MT2OFX Input Processing Script Mint.com CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/MintUS-CSV.vbs 1     17/01/11 1:19 Colin $"
...
With Params
	...
	.ScriptName = "MintUS-CSV"
	.FormatName = "Mint.com CSV"
...
' "12/20/2010","Lowe's","LOWE'S #1125 STERLING VA POS PURC /  HASE/PIN DDA 122010","15.23","debit","Home Improvement","NOW ACCOUNT","",""
	.Fields = Array( _
        Array(fldBookDate, "Date"), _
        Array(fldPayee, "Description"), _	
        Array(fldMemo, "Original Description"), _	
        Array(fldAmount, "Amount"), _
        Array(fldUser1, "Transaction Type"), _
        Array(fldCategory, "Category"), _
        Array(fldAccountNum, "Account Name"), _
        Array(fldSkip, "Labels"), _
        Array(fldSkip, "Notes") _
	)
...
'	.MaxFieldsExpected = 1
	.Properties = Array( _
        ...
        Set .User1Callback = GetRef("TxnTypeCallback")
...
' MsgBox "In transaction callback: " & t.Memo
    If t.Category = "Uncategorized" Then t.Category=""
...
'msgbox "setting context to " & vfields(7)
    SetContext vFields(7)
...
Function TxnTypeCallback(s, t, sField)
    If sField = "debit" Then
        t.Amt = -t.Amt
    End If
    TxnTypeCallback = True
End Function
