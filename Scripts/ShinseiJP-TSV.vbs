' MT2OFX Input Processing Script Shinsei Bank (Japan) TSV format
' NB: Input file is Unicode in UTF-16 with Japanese text in the Description!

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/ShinseiJP-TSV.vbs 1     20/06/09 14:11 Colin $"
...
With Params
	...
	.ScriptName = "ShinseiJA-CSV"
	.FormatName = "Shinsei Bank (Japan) TSV"
	...
	.CurrencyCode = "JPY"	' default if not specified in file
	.ColumnHeadersPresent = True	' are the column headers in the file?
	.SkipHeaderLines = 8	' number of lines to skip before the transaction data
...
' to the text between the top-level parentheses.
	...
	.MonthNames = Empty
' Value Date	Ref No.	Description	DR	CR	Running Balance
	.Fields = Array( _
		Array(fldValueDate, "Value Date"), _
		Array(fldSkip, "Ref No."), _
		Array(fldMemo, "Description"), _
		Array(fldAmtDebit, "DR"), _
		Array(fldAmtCredit, "CR"), _
...
' MsgBox "In header callback: " & sLine
	If Len(sLine) > 0 Then
		Dim vFields: vFields = Split(sLine, vbTab)
		If vFields(0) = "Activity For" Then
			Params.AccountNum = vFields(2)
		ElseIf vFields(0) = "Current Balance" Then
