' MT2OFX Input Processing Script HSBC (UK) CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/HsbcUK-CSV.vbs 1     17/04/09 23:22 Colin $"
...
With Params
	...
	.ScriptName = "HsbcUK-CSV"
	.FormatName = "HSBC (UK) CSV"
...
' to the text between the top-level parentheses.
	...
	.MonthNames = Empty
' 06/04/2009,ADVICE CONFIRMS 4A06049S80206867 ECHANGES CULTUREL,2906
	.Fields = Array( _
...
'	.MaxFieldsExpected = 1
	.Properties = Array( _
		Array("AcctNum", "Account number", _
			...
			ptString,,"=CheckAccount", "Please enter an eight-digit account number.") _
		)
...
'MsgBox "In transaction callback: " & t.Memo
	If StartsWith(t.Memo, "ADVICE CONFIRMS ") Then
		t.Payee = Trim(Mid(t.Memo, 33))
	Else
		t.Payee = t.Memo
	End If
