' MT2OFX Input Processing Script Northern Trust CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/NorthernTrustUS-CSV.vbs 3     25/11/08 22:23 Colin $"

Const ScriptName = "NorthernTrustUS-CSV"
Const FormatName = "Northern Trust CSV Format"
...
'Account Number,Account Name/Nickname,Date,Type,Description,Amount
Dim aFields
aFields = Array( _
	Array(fldAccountNum, "Account Number"), _
	Array(fldSkip, "Account Name/Nickname"), _
...
Function LoadTextFile()
	...
	Dim dMinDate
	Dim dOpenBalDate, dOpenBalAmt: dOpenBalDate = NODATE
	Dim FirstTxn
	...
	Do While Not AtEOF()
		sLine = ReadLine()
' NorthernTrust: for some reason there is sometimes a backslash before the comma between the description and the memo
		sLine = Replace(sLine, "\,", ",")
...
' Northern Trust: special stuff
			Select Case vFields(FieldDict(fldSkip))
			Case "EFT"
				Txn.Payee = Trim(Left(sMemo, 16))
			Case "Deposit"
				' not much we can do with this
			Case "Check"
				Txn.TxnType = "CHECK"
				If StartsWith(sMemo, "CHECK ") Or StartsWith(sMemo, "SUBSTITUTE CHECK ") Then
					Txn.CheckNum = Trim(Right(sMemo, 6))
				End If
			Case "Other"
			End Select
...
' keep tabs on the statement/balance Date
			...
			FirstTxn = False
		ElseIf Left(sLine, 1) = CSVSeparator Then
' ,,12/31/2007,Beginning Balance,,685589.92
' ,,01/31/2008,Ending Balance,,454333.24
			vFields = ParseLineDelimited(sLine, CSVSeparator)
			If TypeName(vFields) <> "Variant()" Then
				MsgBox ParseErrorMessage, vbOkOnly+vbCritical, ParseErrorTitle
				Abort
...
' as this precedes the first txn, we have to remember it until the Statement is created
				dOpenBalDate = ParseDate(vFields(3))
				dOpenBalAmt = ParseNumber(vFields(6), ".")
			Case "Ending Balance"
				Stmt.ClosingBalance.BalDate = ParseDate(vFields(3))
				Stmt.ClosingBalance.Amt = ParseNumber(vFields(6), ".")
			End Select
		End If
	Loop
	If dOpenBalDate <> NODATE Then
		Stmt.OpeningBalance.BalDate = dOpenBalDate
		Stmt.OpeningBalance.Amt = dOpenBalAmt
	End If
	LoadTextFile = True
End Function
