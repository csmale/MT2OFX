...
Const ScriptVersion = "$Header: /MT2OFX/VolksbankDE-CSV.vbs 7     30/01/11 13:35 Colin $"

Const ScriptName = "VolksbankDE-CSV"
Const FormatName = "Volksbank (Germany) CSV"
...
Const BankCode = "GENODE61RAD"	' is Volksbank EG, Konstanz
...
Dim NumFieldsExpected: NumFieldsExpected = 10
...
Const SkipHeaderLines = 12	' number of lines to skip before the transaction data
...
'"01.02.2010";"31.01.2010";;;;;"ABSCHLUSS

'ABSCHLUSS PER 31.01.2010";"EUR";"3,00";"S"
...
aFieldsOld = Array( _
    ...
    Array(fldMemo, "Vorgang/Verwendungszweck"), _
    Array(fldCurrency, ""), _
    Array(fldAmount, "Umsatz"), _
    Array(fldSkip, "") _
)
...
aFields = Array( _
    Array(fldBookDate, "Buchungstag"), _
    Array(fldValueDate, "Valuta"), _
    Array(fldMemo, "Auftraggeber/Zahlungsempfänger"), _
    Array(fldPayee, "Empfänger/Zahlungspflichtiger"), _
    Array(fldMemo, "Konto-Nr."), _
    Array(fldMemo, "BLZ"), _
    Array(fldMemo, "Vorgang/Verwendungszweck"), _
...
Function GetLine()
    ...
    Do While Not AtEOF()
        ...
        If Right(sTmp, 1) = """" Then
            ...
            Exit Function
        End If
        If Len(sLine) = 0 And Len(sTmp) = 0 Then
            Exit Function
        End If
        If Len(sLine) > 0 And Len(sSavedPayee) = 0 Then
            sSavedPayee = sTmp
        End If
        sLine = sLine & sTmp
    Loop
...
Function RecogniseTextFile()
	...
	For i=1 To SkipHeaderLines
		...
		End If
		sLine = ReadLine()
		If i=1 and Instr(sLine, "Volksbank") = 0 Then
            If DebugRecognition Then
                MsgBox "Didn't find Volksbank in line 1"
            End If
            Exit Function
		End if
		If i=3 And Instr(sLine, "Umsatzanzeige") = 0 Then
            If DebugRecognition Then
                MsgBox "Didn't find Umsatzanzeige in line 3"
            End If
            Exit Function
		End If
	Next
	...
	If ColumnHeadersPresent Then
		For i=1 To NumFieldsExpected
			If vFields(i) <> aFields(i-1)(1) Then
				If DebugRecognition Then
					MsgBox "field " & CStr(i) & " expecting '" & aFields(i-1)(1) & "' got '" & vFields(i) & "'"
				End If
				Exit function
			End if
		Next
...
' Volksbank: get bank account and branch from headers
		If InStr(sLine, "BLZ:") > 0 Then
			vFields = ParseLineDelimited(sLine, CSVSeparator)
			BankBranch = vFields(2)
		Elseif InStr(sLine, "Konto:") > 0 Then
			vFields = ParseLineDelimited(sLine, CSVSeparator)
	...
	' set up new transaction, and start a new statement if the account # changes
		...
		' this initialisation should be in the class constructor!! (fixed in 3.3.5)
			...
			For i=1 To UBound(vFields)
				...
				End select
			Next
' Volksbank: correct sign of amount according to last field (6 or 10): S=credit, H=debit
			If vFields(NumFieldsExpected) = "S" Then
				Txn.Amt = -Txn.Amt
			End If
...
'    msgbox "got opening balance: " & sLine
			vFields = ParseLineDelimited(sLine, CSVSeparator)
			Stmt.OpeningBalance.BalDate = ParseDate(vFields(1))
			Stmt.OpeningBalance.Amt = ParseNumber(vFields(NumFieldsExpected-1), DecimalSeparator)
			If vFields(NumFieldsExpected) = "S" Then
				Stmt.OpeningBalance.Amt = -Stmt.OpeningBalance.Amt
			End If
		ElseIf InStr(sLine, "Endsaldo") > 0 Then
'    msgbox "got closing balance: " & sLine
			vFields = ParseLineDelimited(sLine, CSVSeparator)
			Stmt.ClosingBalance.BalDate = ParseDate(vFields(1))
			Stmt.ClosingBalance.Amt = ParseNumber(vFields(NumFieldsExpected-1), DecimalSeparator)
			If vFields(NumFieldsExpected) = "S" Then
