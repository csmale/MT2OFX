' MT2OFX Input Processing Script Royal Bank of Canada CSV format
...
With Params
	...
	.ScriptName = "RbcCA-CSV"
	.FormatName = "Royal Bank of Canada CSV"
	.ParseErrorMessage = "Cannot parse line."
	.ParseErrorTitle = .ScriptName
	.BankCode = "ROYCCAT2MIC"
...
' to the text between the top-level parentheses.
	...
	.MonthNames = Empty
' 00000003,01871,001011010,20090302,20090302,000,000,"CAD",0,00464,0000000000000,
'  "PAIEMENT DIVERS     ","RBC                 ",0000000083.72,0000000000.00,0000000000.00,0000000000.00
	.Fields = Array( _
		Array(fldSkip, "FI Code", "3"), _
		Array(fldBranch, "Routing code", "\d{5}"), _
		Array(fldAccountNum, "Account", "\d{9}"), _
		Array(fldBookDate, "Book Date"), _
		Array(fldValueDate, "Value Date"), _
		Array(fldSkip, "Unknown", "000"), _
		Array(fldSkip, "Unknown", "000"), _
		Array(fldCurrency, "Currency"), _
		Array(fldSkip, "Debit(0)/Credit(1)", "0|1"), _
		Array(fldSkip, "Trans code"), _
		Array(fldCheckNum, "Unknown", "\d{13}"), _
		Array(fldMemo, "Memo 1"), _
		Array(fldPayee, "Memo 2"), _
		Array(fldAmount, "Amount (always positive)"), _
		Array(fldSkip, "Unknown"), _
		Array(fldSkip, "Unknown"), _	
		Array(fldSkip, "Unknown") _	
	)
...
'MsgBox "In transaction callback: " & t.Memo
	Dim iChk
	If IsNumeric(t.CheckNum) Then
		iChk = CLng(t.CheckNum)
		If iChk = 0 Then
			t.CheckNum = ""
		Else
			t.CheckNum = Right(t.CheckNum, 12)	' OFX: A-12
			t.TxnType = "CHECK"
		End If
	End If
...
' sort out debit/credit
	If vFields(9) = "0" Then
		vFields(14) = "-" & vFields(14)
	End If
