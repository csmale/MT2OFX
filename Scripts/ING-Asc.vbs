' MT2OFX Input Processing Script for ING Bank NL Asc format

Option Explicit

Private Const ScriptVersion = "$Header: /MT2OFX/ING-Asc.vbs 2     17/07/04 0:19 Colin $"

Const ScriptName = "INGBankNL-Asc"
Const FormatName = "ING Bank (NL) Asc-Formaat"
...
Const Pattern = "(\d{2}\.\d{2}\.\d{2}\.\d{3}) (\d{2}-\d{2}-\d{4})  (.{244}) ([ \.\d\-]{12}) (Af |Bij) ([ \d]{17},\d{2})"
...
Function RecogniseTextFile()
	...
	sLine = ReadLine()
	vFields = ParseLineFixed(sLine, Pattern)
...
Function LoadTextFile()
	...
	' set up new transaction, and start a new statement if the account # changes
			...
			If Trim(vFields(fldBijAf)) = "Af" Then
				Txn.Amt = -Txn.Amt
			End If
...
' strip off some trailing rubbish
