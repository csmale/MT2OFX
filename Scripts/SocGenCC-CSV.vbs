' MT2OFX Input Processing Script Société Génerale Credit Card CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/SocGenCC-CSV.vbs 1     5/03/06 23:18 Colin $"

Const ScriptName = "SocGenCC-CSV"
Const FormatName = "Société Génerale Carte Crédit CSV"
Const ParseErrorMessage = "Ne pas possible de traiter cette ligne."
...
Const PayeeLocation = 6		' start of payee in memo
...
'date d'effet;date de compensation;libellé de l'opération;montant de l'opération;devise
'21/04/2005;27/04/2005;2104/COMMERCEPAYMENT. 702 -69,99;-55,42;EUR

Dim aFields
aFields = Array( _
	Array(fldValueDate, "date d'effet"), _
	Array(fldBookDate, "date de compensation"), _
	Array(fldMemo, "libellé de l'opération"), _
...
Function LoadTextFile()
	...
	For i=1 To SkipHeaderLines
		sLine = ReadLine()
' SocGenCC first line has good info!
		If i=1 Then
			vFields = ParseLineDelimited(sLine, CSVSeparator)
' ="4567899865143123000";05/04/2005;;12;06/05/2005;-539,09 EUR;1
			If TypeName(vFields) <> "Variant()" Then
				Exit Function
			End If
...
' this initialisation should be in the class constructor!! (fixed in 3.3.5)
			...
			Stmt.ClosingBalance.Amt = ParseNumber(Left(vFields(6), Len(vFields(6))-4), DecimalSeparator)
			Stmt.ClosingBalance.Ccy = CurrencyCode
			iSeq = 0
			Stmt.BankName = BankCode
			Stmt.Acct = Mid(vFields(1), 3, Len(vFields(1))-3)
		End If			
	Next
	If ColumnHeadersPresent Then
		sLine = ReadLine()
