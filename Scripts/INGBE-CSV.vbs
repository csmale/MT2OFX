' MT2OFX Input Processing Script ING (Belgium) CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/INGBE-CSV.vbs 1     26/11/08 0:01 Colin $"

Const ScriptName = "INGBE-CSV"
Const FormatName = "ING (Belgium) CSV format"
...
' French version
' "Numéro de compte";"Numéro de mouvement";"Date comptable";"Date valeur";"Montant dans la devise du compte";"Devise du compte";
' "Montant dans la devise de contre-valeur";"Devise de contre-valeur";"Rubriques";"Libellés";"Détails";"Remarques";
' "Date de comptabilisation en budget"
' Dutch version
' "Rekeningnummer";"Omzetnummer";"Boekingsdatum";"Valutadatum";"Bedrag in de munteenheid van de rekening";"Munteenheid van de rekening";
' "Bedrag in de tegenwaarde";"Munteenheid van de tegenwaarde";"Rubrieken";"Omschrijving";"Detail";"Opmerkingen";
' "Boekingsdatum in budget"
Dim aFieldsNl
aFieldsNl = Array( _
	Array(fldAccountNum, "Rekeningnummer"), _
	Array(fldSkip, "Omzetnummer"), _
	Array(fldBookDate, "Boekingsdatum"), _
	Array(fldValueDate, "Valutadatum"), _
	Array(fldAmount, "Bedrag in de munteenheid van de rekening"), _
	Array(fldCurrency, "Munteenheid van de rekening"), _
	Array(fldSkip, "Bedrag in de tegenwaarde"), _
	Array(fldSkip, "Munteenheid van de tegenwaarde"), _
	Array(fldSkip, "Rubrieken"), _
	Array(fldMemo, "Omschrijving"), _
	Array(fldMemo, "Detail"), _
	Array(fldMemo, "Opmerkingen"), _
	Array(fldSkip, "Boekingsdatum in budget")  _	
)
Dim aFieldsFr
aFieldsFr = Array( _
	Array(fldAccountNum, "Numéro de compte"), _
	Array(fldSkip, "Numéro de mouvement"), _
	Array(fldBookDate, "Date comptable"), _
	Array(fldValueDate, "Date valeur"), _
	Array(fldAmount, "Montant dans la devise du compte"), _
	Array(fldCurrency, "Devise du compte"), _
	Array(fldSkip, "Montant dans la devise de contre-valeur"), _
	Array(fldSkip, "Devise de contre-valeur"), _
	Array(fldSkip, "Rubriques"), _
	Array(fldMemo, "Libellés"), _
	Array(fldMemo, "Détails"), _
	Array(fldMemo, "Remarques"), _
	Array(fldSkip, "Date de comptabilisation en budget")  _	
)
Dim aFields: aFields = aFieldsNl	' assume Dutch, switch to French if needed
...
Function RecogniseTextFile()
	...
	If ColumnHeadersPresent Then
		If vFields(1) = "Numéro de compte" Then
			aFields = aFieldsFr
		Else
			aFields = aFieldsNl
		End If
...
Function LoadTextFile()
	...
	If ColumnHeadersPresent Then
		sLine = ReadLine()
		If StartsWith(sLine, "Numéro de compte;") Then
			aFields = aFieldsFr
		Else
			aFields = aFieldsNl
		End If
		...
		' this initialisation should be in the class constructor!! (fixed in 3.3.5)
			...
			For i=1 To UBound(vFields)
				...
				case fldSkip, fldEmpty
					' do nothing
				case fldAccountNum
	' ING Belgium: trim trailing stuff
					sField = Left(sField, 14)
					Stmt.Acct = sField
					sAcct = sField
				case fldCurrency
