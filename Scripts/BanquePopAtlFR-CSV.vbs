...
Const ScriptVersion = "$Header: /MT2OFX/BanquePopAtlFR-CSV.vbs 1     19/10/10 1:07 Colin $"
...
'Compte;Date de comptabilisation;Date opération;Libellé;Référence;Date valeur;Montant
	.Fields = Array( _
		...
		Array(fldTransactionDate, "Date opération"), _
		Array(fldMemo, "Libellé"), _
		Array(fldMemo, "Référence"), _
...
'MsgBox "In transaction callback: " & t.Memo
    If StartsWith(t.Memo, "CARTE ") Then
        t.TxnType = "POS"
        t.Payee = Trim(Mid(vFields(4),6))
    ElseIf StartsWith(t.Memo, "RETRAIT ") Then
        t.TxnType = "ATM"
    ElseIf StartsWith(t.Memo, "PRLV ") Then
        t.TxnType = "DIRECTDEBIT"
        t.Payee = Trim(Mid(vFields(4),5))        
    End If
