...
Const ScriptVersion = "$Header: /MT2OFX/HoaresUK-CSV.vbs 5     21/06/09 21:22 Colin $"

Const ScriptName = "HoaresUK-CSV"
Const FormatName = "C Hoare's Bank UK CSV"
...
aPropertyList = Array( _
	Array("AcctNum", "Account Number", _
		"The account number to be placed in the output file.", _
		ptString) _
	)
...
Function LoadTextFile()
	...
	LoadTextFile = False
	AccountNum = CStr(GetProperty("AcctNum"))
	For i=1 To SkipHeaderLines
		sLine = ReadLine()
	Next
	If ColumnHeadersPresent Then
...
' keep tabs on the statement/balance Date
			...
			End If
' Hoares: transactions in ascending date order, therefore first transaction gives opening balance info!
