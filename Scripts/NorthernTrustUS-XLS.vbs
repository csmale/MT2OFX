' MT2OFX Input Processing Script Northern Trust (USA) XLS format
Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/NorthernTrustUS-XLS.vbs 1     6/06/09 23:00 Colin $"

Const ScriptName = "NorthernTrustUS-XLS"
Const FormatName = "Northern Trust (US) XLS"
...
Const CurrencyCode = "USD"	' default if not specified in file
Const NoAvailableBalance = False		' True if file does not contain "Available Balance" information
...
' n_bk_abbr	i_bls_ac	c_tran_type	s_as_of	n_ac_title	c_bls_bai	i_tran_cnt	a_dol	t_tran_desc1	t_tran_desc2	t_tran_desc3	t_tran_desc4	t_tran_desc5	t_tran_desc6	t_tran_desc7
' NTNA	1016043347	CR	3/2/09  12:00:00 AM	GASTROCARE SWEEP ACCOUNT	142		78.29	AETNA LIFE INS    HMOPYMT1          705TRN*3*160225090002343  714623810\
Dim aFields
aFields = Array( _
	Array(fldSkip, "n_bk_abbr"), _
	Array(fldAccountNum, "i_bls_ac"), _
	Array(fldSkip, "c_tran_type"), _
	Array(fldBookDate, "s_as_of"), _
	Array(fldSkip, "n_ac_title"), _
	Array(fldSkip, "c_bls_bai"), _
	Array(fldSkip, "i_tran_cnt"), _
	Array(fldAmount, "a_dol"), _
	Array(fldMemo, "t_tran_desc1"), _
	Array(fldMemo, "t_tran_desc2"), _
	Array(fldMemo, "t_tran_desc3"), _
	Array(fldMemo, "t_tran_desc4"), _
	Array(fldMemo, "t_tran_desc5"), _
	Array(fldMemo, "t_tran_desc6"), _
	Array(fldMemo, "t_tran_desc7") _
)
...
Function InitXLS()
	...
	If Ucase(Right(Session.InputFile.FileName, 4)) <> ".XLS" Then
		DebugMessage "Not an XLS file"
		Exit Function
	End If
	ixRow = 0
	If Not (xExcel Is Nothing) Then
		InitXLS = True
		DebugMessage "xExcel not nothing"
		Exit Function
	End If
	...
	End If
	If xExcel Is Nothing Then
		DebugMessage "Unable to create Excel.Application"
		Exit Function
	End If
	DebugMessage TypeName(xExcel)
	Set xDocument = xExcel.Workbooks.Open(Session.InputFile.FileName, False, True)
	If xDocument Is Nothing Then
		Set xExcel = Nothing
			DebugMessage "Unable to open file " & Session.InputFile.FileName & " in Excel: " & Err.Description
		Exit Function
	End If
	Set xSheet = xDocument.Worksheets(1)
	If xSheet Is Nothing Then
		DebugMessage "Unable to find worksheet in Excel document"
		xDocument.Close
...
Sub DebugMessage(sMsg)
	If DebugRecognition Then
		Message True, True, sMsg, ScriptName
	End If
End Sub
...
Function RecogniseTextFile()
	...
	If Not InitXLS() Then
msgbox "InitXLS failed"
		Exit Function
	End If
	For i=1 To SkipHeaderLines
		If AtEOFXLS() Then
MsgBox "EOF skipping header lines"
			Exit Function
		End If
		vFields = ReadLineXLS()
	Next
	If AtEOFXLS() Then
MsgBox "EOF after header lines"
		Exit Function
	End If
	vFields = ReadLineXLS()
			...
			If UBound(aFields(i-1)) > 1 Then
				...
				Else
					If Not StringMatches(sField, aFields(i-1)(2)) Then
						DebugMessage "Field " & CStr(i) & ": '" & sField & "' does not match '" & aFields(i-1)(2) & "'"
						Exit Function
					End If
				End If
			Else
				If sField <> aFields(i-1)(1) Then
					DebugMessage "Field " & CStr(i) & " " & sField & ", expecting " & aFields(i-1)(1)
					Exit Function
				End If
			End If
		Next
...
'	If sAcct="" Then sAcct = AccountNum
	...
	Do While Not AtEOFXLS()
		...
		End If
		If UBound(vFields) < MinFieldsExpected Or UBound(vFields) > MaxFieldsExpected Then
			MsgBox "Wrong number of fields - " & CStr(UBound(vFields)+1) & " - " & sLine, ScriptName
			Abort
			Exit function
		End If
	...
	' this initialisation should be in the class constructor!! (fixed in 3.3.5)
			...
			FirstTxn = True
		End If
		If vFields(3) <> "CR" And vFields(3) <> "DB" Then	' only look at lines with CR or DB here
			If vFields(9) = "OPENING LEDGER BALANCE" Then
				If Stmt.OpeningBalance.BalDate = NODATE Then
					Stmt.OpeningBalance.BalDate = vFields(4)
					Stmt.OpeningBalance.Amt = vFields(8)
				End If
			ElseIf vFields(9) = "CLOSING LEDGER BALANCE" Then
				Stmt.ClosingBalance.BalDate = vFields(4)
				Stmt.ClosingBalance.Amt = vFields(8)
			ElseIf vFields(9) = "CLOSING AVAILABLE BALANCE" Then
				Stmt.AvailableBalance.BalDate = vFields(4)
				Stmt.AvailableBalance.Amt = vFields(8)
			End If
			' do nothing
		Else
			NewTransaction
...
' find the payee, transaction type and txn date if we can
			...
			End If
			If vFields(6) = 142 Then
				Txn.Payee = Trim(Left(Txn.Memo, 18))
			End If
