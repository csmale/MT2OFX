...
Const ScriptVersion = "$Header: /MT2OFX/alliance&LeicesterUK-CSV.vbs 1     10/03/07 22:18 Colin $"

Const ScriptName = "Alliance&LeicesterUK-CSV"
Const FormatName = "Alliance & Leicester (UK) CSV"
...
Const TxnsReversed = True			' True if the oldest transaction is last in the file
...
aPropertyList = Array( _
	Array("ALAccount", "Account number", _
		"Please enter the number of your account with Alliance & Leicester, as eight digits.", _
		ptString,,"=IsValidAccountNumber") _
	)
...
Function LoadTextFile()
	...
	LoadTextFile = False
	AccountNum = GetProperty("ALAccount")
	If Len(AccountNum) = 0 Then
		AccountNum = GetAccountNumber("ALAccount", _
			"Please enter your Alliance&Leicester account number, as eight digits.", _
			"Unknown account", _
			"IsValidAccountNumber", _
			aPropertyList, ScriptName)
			If Len(AccountNum) = 0 Then
				Abort
				Exit Function
			End If
	...
	' set up new transaction, and start a new statement if the account # changes
		...
		' this initialisation should be in the class constructor!! (fixed in 3.3.5)
			...
			For i=1 To UBound(vFields)
				...
				case fldClosingBal
					If (Not TxnsReversed) Or (Stmt.ClosingBalance.BalDate = NODATE) Then
...
' Alliance & Leicester
			If StartsWith(sMemo, "DIRECT DEBIT ") Then
				Txn.Payee = Mid(sMemo, 13)
			ElseIf StartsWith(sMemo, "CASH MACHINE ") Then
				Txn.Payee = "Cash Withdrawal"
				Txn.TxnType = "ATM"
			ElseIf StartsWith(sMemo, "BANK CREDIT ") Then
				Txn.Payee = Mid(sMemo, 13)
				Txn.TxnType = "CREDIT"
			ElseIf StartsWith(sMemo, "CHEQUE DEPOSIT ") Then
				Txn.TxnType = "DEP"
			ElseIf StartsWith(sMemo, "CHEQUE ") Then
				Txn.TxnType = "CHECK"
				Txn.CheckNum = Mid(sMemo, 8)
			ElseIf StartsWith(sMemo, "B-PAYMNT TO ") Then
				Txn.TxnType = "PAYMENT"
				Txn.Payee = Mid(sMemo, 13)
			ElseIf StartsWith(sMemo, "GROSS INTEREST") Then
				Txn.TxnType = "INT"
				Txn.Payee = "Gross Interest"
			ElseIf StartsWith(sMemo, "NET INTEREST") Then
				Txn.TxnType = "INT"
				Txn.Payee = "Net Interest"
			End If
...
Function GetAccountNumber(sProp, sPrompt, sTitle, fValidate, aPropSet, sScriptName)
	Dim sAcct
	Dim sFunc
	Dim bChanged
	If Len(fValidate) > 0 Then
		sFunc = fValidate & "(sAcct)"
	Else
		sFunc = "True"
	End If
	sAcct = GetProperty(sProp)
	bChanged = False
	Do While Not Eval(sFunc)
' the following line should use PromptForInput (from v3.5.3) in order to kill the progress bar
		sAcct = InputBox(sPrompt, sTitle, sAcct)
		If Len(sAcct) = 0 Then
			bChanged = False
			Exit Do
		Else
...
' the next line needs a fix in the executable (from v3.5.3)
'		SaveProperties sScriptName, aPropSet
	End If
	GetAccountNumber = sAcct
End Function

' IsValidAccountNumber is called by GetAccountNumber located in MT2OFX.vbs
' and is used as a validator for property editing
Function IsValidAccountNumber(sAcct)
	IsValidAccountNumber = (Len(sAcct) = 8) And (IsNumeric(sAcct))
End Function
