' MT2OFX Input Processing Script Eik Bank (DK) CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/EikBankDK-CSV.vbs 2     11/05/10 23:55 Colin $"
...
With Params
	...
	.ScriptName = "EikBankDK-CSV"
	.FormatName = "Eik Bank (DK) CSV"
	...
	.CurrencyCode = "DKK"	' default if not specified in file
...
' MsgBox "In transaction callback: " & t.Memo
    Dim iTmp
    If StartsWith(t.Memo, "DK-NOTA") Then
        t.Payee = Mid(t.Memo, 13)
    ElseIf StartsWith(t.Memo, "GOOGLE") Or StartsWith(t.Memo, "PAYPAL") Then
        iTmp = InStr(t.Memo, "*")
        If iTmp > 0 Then
            t.Payee = Mid(t.Memo, iTmp+1)
        Else
            t.Payee = t.Memo
        End If
    ElseIf IsNumeric(Left(t.Memo, 3)) And Mid(t.Memo, 4, 1) = " " Then
        t.TxnType = "ATM"
    Else
        t.Payee = t.Memo
    End If
