' MT2OFX Input Processing Script Citicard CA CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/CiticardCA-CSV.vbs 2     25/11/08 22:14 Colin $"

' Thanks to Bert Wensley!

Const ScriptName = "CiticardCA-CSV"
Const FormatName = "Citicard (CA) CSV"
...
Const PayeeLength = 22		' length of payee in memo
...
aFields = Array( _
	...
	Array(fldBookDate, "Posting Date"), _
...
aPropertyList = Array( _
	Array("CardNumber", "Citicard Number", _
		"Citicard Number with no spaces.", _
		ptString) _
	)
...
Function LoadTextFile()
	...
	Dim dMinDate	' earliest txn/book date - if we don't have a statement date
	...
	Do While Not AtEOF()
		...
		If Len(sLine) > 0 And Left(sLine,1) <> CSVSeparator And _
			(IsNumeric(Left(sLine, 1)) Or (Left(sLine, 1)="""" And IsNumeric(Mid(sLine,2,1)))) Then
			If CSVSeparator = "" Then
				vFields = ParseLineFixed(sLine, TxnLinePattern)
			Else
				vFields = ParseLineDelimited(sLine, CSVSeparator)
	...
	' set up new transaction, and start a new statement if the account # changes
		...
		' this initialisation should be in the class constructor!! (fixed in 3.3.5)
					...
					iSeq = 0
					Stmt.BankName = BankCode
					Stmt.Acct = GetProperty("CardNumber")
					dMaxDate = NODATE
					dMinDate = NODATE
				End If
			End If
...
' find the payee, transaction type and txn date if we can
			Txn.Memo = Replace(Txn.Memo, "&grave;", "'")	' not a standard entity!
'			Txn.Memo = Replace(Txn.Memo, "&amp;", "&")		' for V3.4
			Txn.Memo = HTMLDecode(Txn.Memo)					' for V3.5 and later
			sMemo = Txn.Memo
