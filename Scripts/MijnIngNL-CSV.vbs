' MT2OFX Input Processing Script MijnING CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/MijnIngNL-CSV.vbs 1     16/02/10 8:26 Colin $"
...
With Params
	...
	.ScriptName = "MijnING-CSV"
	.FormatName = "MijnING CSV"
...
'Datum;Naam / Omschrijving;Rekening;Tegenrekening;Code;Af Bij; Bedrag (EUR) ;MutatieSoort;Mededelingen
	.Fields = Array( _
		...
		Array(fldPayee, "Naam / Omschrijving"), _
		Array(fldAccountNum, "Rekening"), _
		Array(fldSkip, "Tegenrekening"), _
		Array(fldSkip, "Code"), _
		Array(fldSkip, "Af Bij"), _
		Array(fldAmount, "Bedrag (EUR)"), _
		Array(fldSkip, "MutatieSoort"), _
		Array(fldMemo, "Mededelingen") _
	)
...
'MsgBox "In transaction callback: " & t.Memo
    If vFields(6) = "Af" Then
        t.Amt = -t.Amt
    End If
    t.TxnType = TransType(vFields(5), t.Amt)
    
			' direct debits almost always have the payee name in the memo field
    If vFields(5) = "IC" Then
        If StartsWith(t.Payee, "KN: ") Then
            t.Payee = Trim(Left(vFields(9), 24))
        End If
    End If
