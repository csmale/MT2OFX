' MT2OFX Input Processing Script SkandiaBank (DK) CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/SkandiaDK-CSV.vbs 1     4/04/07 22:00 Colin $"

Const ScriptName = "SkandiaBank-CSV"
Const FormatName = "SkandiaBank (Denmark) CSV Format"
...
' 02-01-2007;"INSPEKT SECURITY - L"; 20503,70; 28138,61;"02-01-2007";
Dim aFields
aFields = Array( _
...
aPropertyList = Array( _
	Array("AcctNum", "Account number", _
		"The Skandia Bank and Account number (Reg-konto-nr) as 14 digits.", _
...
' find the payee, transaction type and txn date if we can
			sMemo = Txn.Memo
			If StartsWith(sMemo, "DK-NOTA") Then
				Txn.Payee = Trim(Mid(sMemo, 14))
			ElseIf StartsWith(sMemo, "DK-KØB") Then
				Txn.Payee = Trim(Mid(sMemo, 14))
			Else
				Txn.Payee = sMemo
			End If
