' MT2OFX Input Processing Script Bank Leumi (Israel) DAT (CSV) format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/BankLeumiIL-DAT.vbs 1     25/11/08 23:52 Colin $"

Const ScriptName = "BankLeumiIL-CSV"
Const FormatName = "Bank Leumi (Israel) DAT (CSV) format"
Const ParseErrorMessage = "Cannot parse line."
Dim ParseErrorTitle : ParseErrorTitle = ScriptName
Const MinimumProgramVersion = "3.5.4"

Const DebugRecognition = False	' enables debug code in recognition
Const BankCode = "Leumi"
...
' NB: The input files are in DOS (OEM) Code Page 862, not Windows 1255!
' 0026000,150207,"<hebrew text>",-000000205.00,-000037649.48,0003,80533003486543
'The fields, from left to right, are
'1.       Booking code for the transaction
'2.       Value Date DDMMYY
'3.       Memo (Hebrew characters between inverted commas)
'4.       Amount
'5.       Balance
'6.       Statement Page number
'7.       Account number
Dim aFields
aFields = Array( _
	Array(fldSkip, "Booking code"), _
	Array(fldValueDate, "Value date"), _
	Array(fldMemo, "Memo"), _
	Array(fldAmount, "Amount"), _
	Array(fldClosingBal, "Balance"), _
	Array(fldSkip, "Statement page number"), _
...
' input file uses DOS Hebrew code page (862)
	Session.InputFile.CodePage = 862
