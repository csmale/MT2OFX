...
Const ScriptVersion = "$Header: /MT2OFX/NorisbankDE-CSV.vbs 1     21/10/09 0:00 Colin $"

Const ScriptName = "NorisbankDE-CSV"
Const FormatName = "Norisbank CSV format"
...
aFields = Array( _
	...
	Array(fldCurrency, "Waehrung", "Waehrung|Währung") _	
)
...
Function RecogniseTextFile()
    ...
    For i=1 To SkipHeaderLines
        ...
        End If
        sLine = ReadLine()
        If StartsWith(sLine, "Buchungstag") Then
            Exit For
        End If
	Next
	If AtEOF() Then
		Exit Function
	End If
    If Not StartsWith(sLine, "Buchungstag") Then
        sLine = ReadLine()
    End If
        ...
        For i=1 To UBound(vFields)
            If UBound(aFields(i-1)) > 1 Then
                If Not StringMatches(vFields(i), aFields(i-1)(2)) Then
                    If DebugRecognition Then
                        MsgBox "field " & CStr(i) & " '" & vFields(i) & "' doesn't match '" & aFields(i-1)(2) & "'",,ScriptName
                    End If
                    Exit function
                End If
            Else
