' MT2OFX Input Processing Script La Banque Postale CSV format
...
With Params
	...
	.ScriptName = "LaBanquePostaleFR-CSV"
	.FormatName = "La Banque Postale (France) CSV"
...
'Date;Libellé;Montant(EUROS);Montant(FRANCS)
	.Fields = Array( _
		...
		Array(fldAmount, "Montant(EUROS)"), _
		Array(fldSkip, "Montant(FRANCS)") _
	)
...
'MsgBox "In header callback: " & sLine
	Dim vFields
	HeaderCallback = False
	If Len(sLine) = 0 Then
		Exit Function
	End If
	vFields = Split(sLine, Params.CSVSeparator)
	If UBound(vFields) <> 1 Then
		Exit Function
	End If
	Select Case Trim(vFields(0))
	Case "Numéro Compte"
'MsgBox "Account number=" & vFields(1)
		Params.AccountNum = Trim(vFields(1))
	Case "Date"
MsgBox "Balance Date=" & vFields(1)
		BalDate = Params.ParseDate(vFields(1))
	Case "Solde (EUROS)"
'MsgBox "Balance Amount=" & vFields(1)
		BalAmt = Params.ParseAmount(vFields(1))
	End Select
	HeaderCallback = True
End Function
...
'CHEQUE N°8874038
	If StartsWith(t.Memo, "CHEQUE N°") Then
		t.CheckNum = Mid(t.Memo, 10)
		t.TxnType = "CHECK"
	ElseIf StartsWith(t.Memo, "FRAIS ") Then
		t.TxnType = "SRVCHG"
	End If
...
MsgBox "bal date = " & BalDate
	Stmt.ClosingBalance.BalDate = BalDate
	Stmt.ClosingBalance.Amt = BalAmt
