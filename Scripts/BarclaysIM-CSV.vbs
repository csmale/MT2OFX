' MT2OFX Input Processing Script Barclays (Isle of Man) CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/BarclaysIM-CSV.vbs 1     21/08/09 19:01 Colin $"
...
With Params
	...
	.ScriptName = "BarclaysIM-CSV"
	.FormatName = "Barclays Private Clients International (Isle of Man) CSV"
...
' to the text between the top-level parentheses.
	...
	.MonthNames = Empty
'accountname,sortcodenumber,ibannumber,accounttype,currency,LatestCleared,ClosingBalance,ProjectedBalance,OutstandingTransactions,overdraftlimit,accountheld,balancedate,date,description,details,payment,receipt
	.Fields = Array( _
...
'MsgBox "In transaction callback: " & t.Memo
    Dim iTmp, iSlash
    iSlash = InStr(t.Memo, "/")
    iTmp = InStr(t.Memo, " REF ")
    If iSlash > 0 And iSlash < iTmp Then iTmp = iSlash
    If iTmp > 0 Then
        t.Payee = Left(t.Memo, iTmp-1)
    End If
