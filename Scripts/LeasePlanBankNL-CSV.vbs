' MT2OFX Input Processing Script LeasePlan Bank CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/LeasePlanBankNL-CSV.vbs 2     18/08/10 21:52 Colin $"

Dim Params: Set Params = New MT2OFXScript

Dim dClosingBalance, dOpeningBalance

With Params
	.MinimumProgramVersion = "3"
	.DebugRecognition = False	' enables debug code in recognition
	.ScriptName = "LeasePlanBankNL-CSV"
	.FormatName = "LeasePlan Bank CSV"
   ...
   .CodePage = 1252  ' Windows English / Western Europe
	...
	.SkipHeaderLines = 21	' number of lines to skip before the transaction data
...
' to the text between the top-level parentheses.
	...
	.MonthNames = Empty
' Valutadatum, referentie, beschrijving, debet, credit
	.Fields = Array( _
		Array(fldValueDate, "Valutadatum"), _
		Array(fldSkip, "referentie"), _
		Array(fldMemo, "beschrijving"), _
		Array(fldAmtDebit, "debet"), _
...
' in the class
Function HeaderCallback(ByVal sLine)
'MsgBox "In header callback: " & sLine
    sLine = Replace(sLine, """", "")
    If StartsWith(sLine, "Rekeningnummer") Then
        Params.AccountNum = Trim(Mid(sLine, 15))
    ElseIf StartsWith(sLine, "Rekeningvaluta") Then
        Params.CurrencyCode = Trim(Mid(sLine, 15))
    ElseIf StartsWith(sLine, "Beginsaldo") Then
        dOpeningBalance = ParseNumber(Mid(sLine, 11), ",")
    ElseIf StartsWith(sLine, "Eindsaldo") Then
        dClosingBalance = ParseNumber(Mid(sLine, 10), ",")
    End If
    HeaderCallback = True
End Function
...
' MsgBox "In statement callback"
    Stmt.ClosingBalance.Amt = dClosingBalance
    Stmt.OpeningBalance.Amt = dOpeningBalance
End Sub
