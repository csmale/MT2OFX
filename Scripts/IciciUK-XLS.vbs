' MT2OFX Input Processing Script ICICI (UK) XLS format (which is actually TSV)

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/iciciUK-XLS.vbs 1     3/05/07 22:18 Colin $"

Const ScriptName = "IciciUK-XLS"
Const FormatName = "ICICI Bank (UK) ""XLS"" Format"
...
Const SkipHeaderLines = 11	' number of lines to skip before the transaction data
...
' Txn Srl No.	Txn Date	Description	Txn Type	Txn Amount
Dim aFields
aFields = Array( _
	Array(fldSkip, "Txn Srl No."), _
	Array(fldBookDate, "Txn Date"), _
	Array(fldMemo, "Description"), _
	Array(fldSkip, "Txn Type"), _
	Array(fldAmount, "Txn Amount") _
)
...
Function LoadTextFile()
	...
	Dim dMinDate
	Dim FirstTxn
	Dim iIDSeq, dLastDate	' for FITID generation
	dLastDate = NODATE
	...
	For i=1 To SkipHeaderLines
		sLine = ReadLine()
		If StartsWith(sLine, "Account ID" & vbTab & ": ") Then
			sAcct = Trim(Mid(sLine, 14))
		End If
	Next
	If ColumnHeadersPresent Then
		sLine = ReadLine()
...
' ICICI: FITID based on book Date
			...
			Txn.FITID = CStr(Year(Txn.BookDate)) & "." _
				& Right("000" & CStr(DatePart("y", Txn.BookDate)), 3) & "." _
                & Right("000" & CStr(iIDSeq), 3)
