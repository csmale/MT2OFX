' MT2OFX Input Processing Script SparNebel (DK) CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/SparNebelDK-CSV.vbs 1     4/04/07 22:00 Colin $"

Const ScriptName = "SparNebelDK-CSV"
Const FormatName = "SparNebelDK CSV Format"
...
'"04-04-2007";"03-04-2007";"Dankort Forsberg Nota 01528";"-250,00";"6.235,58"
Dim aFields
aFields = Array( _
...
aPropertyList = Array( _
	Array("AcctNum", "Account number", _
		"The SparNebel Bank and Account number (Reg-konto-nr) as 14 digits.", _
...
Function LoadTextFile()
	...
	Dim dMinDate
	Dim FirstTxn
	Dim re			' RegExp for memo processing
...
' find the payee, transaction type and txn date if we can
			sMemo = Txn.Memo
			If StartsWith(sMemo, "Dankort ") Then
				Txn.Payee = Mid(sMemo, 9)
				re.Pattern = " +Nota \d+$"
				Txn.Payee = re.Replace(Txn.Payee, "")
			End If
