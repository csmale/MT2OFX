' MT2OFX Input Processing Script CitiBank Canada web page clipboard format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/CitibankCA-Web.vbs 3     3/09/05 22:30 Colin $"

Const ScriptName = "CitibankCA-Web"
Const FormatName = "Citibank Canada web page clipboard format"
...
Const BankCode = "CITICATTXXX"
Dim CSVSeparator:CSVSeparator = ""
'31/05	29F66TJR	LONE STAR CAFE-LEMIEUX OTTAWA CAN	29.18
Const HeaderLinePattern = "(.*)\t(.*)\t(.*)\t(.*)"
'Const TxnLinePattern = "(\d\d/\d\d)\t(.{8})\t(.*)\t(\d+\.\d\d-?)"
Const TxnLinePattern = "(\d\d/\d\d)\t(.{8})\t(.*)(?:\t|  +)([\d,]+\.\d\d-?)"
...
Const InvertSign = True	' make credits into debits etc
Const QuickenBankID = 12
Const CurrencyCode = "CAD"	' default if not specified in file
...
' Date	RefNum	Description	Amount
Dim aFields
aFields = Array( _
	Array(fldBookDate, "Date"), _
	Array(fldFITID, "RefNum"), _
...
aPropertyList = Array( _
	Array("NCompte", "Numéro compte CC", _
		"Le numéro du compte carte crédit pour Citibank Canada.", _
		ptString) _
	)
...
' special version for Citibank - no year in dates!
	Dim sTmp
	If Len(sDate) = 5 Then
		sTmp = sDate & DateSeparator & CStr(Year(Now()))
		ParseDate = ParseDateEx(sTmp, DateSequence, DateSeparator)
		If ParseDate <> NODATE And ParseDate > Date() Then
			ParseDate = DateSerial(Year(ParseDate)-1, Month(ParseDate), Day(ParseDate))
		End If
	Else
...
Function RecogniseTextFile()
	...
	If CSVSeparator = "" Then
		If ColumnHeadersPresent Then
			vFields = ParseLineFixed(sLine, HeaderLinePattern)
		Else
	...
	If TypeName(vFields) <> "Variant()" Then
		If DebugRecognition Then
			MsgBox "not var array - it's a " & TypeName(vFields)
		End If
		Exit Function
	End If
...
Function GetAccountNum(sLine)
	Dim sAcct
	Dim sProp, sPrompt
	sAcct = GetProperty("NCompte")
	sPrompt = "Veuillez donner le numéro de compte de votre carte crédit."
	If Len(sAcct) = 0 Then
		sAcct = InputBox(sPrompt, _
			"Citibank Canada - Compte inconnu", _
			sAcct)
		If Len(sAcct) = 0 Then
			Message True, True, "Ne peut pas continuer sans numéro compte!", "Citibank Canada"
			Abort
			Exit Function
		End If
		SetProperty "NCompte", sAcct
		SaveProperties ScriptName, aPropertyList
	End If
	GetAccountNum = sAcct
End Function
