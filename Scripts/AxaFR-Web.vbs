' MT2OFX Input Processing Script Axa Banque France web page clipboard format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/AxaFR-Web.vbs 4     1/03/06 22:19 Colin $"

Const ScriptName = "AxaFR-Web"
Const FormatName = "Axa Banque France web page clipboard format"
...
Const BankCode = "AXABFRPPXXX"
Const CSVSeparator = ""
Const TxnLinePattern = "(\d\d/\d\d/\d{2,4})(?: -)? (.*?) ([-+]?\d[\d ]*,\d\d) *(Eur|[-+]?\d[\d ]*,\d\d) *"
' 16/08/2005 -  CHEQUE N 3802355  -111,39 Eur
' new format (december 2005) for non-CC account below
' 07/12/05 PREL AXA BFIN00 000000000 ... -299,10  -165,40

Const NumFieldsExpected = 4
...
Const TxnDatePattern = "(\d\d)/(\d\d) .*"	' pattern to find transaction date in the memo
...
Dim aFields
aFields = Array( _
	Array(fldBookDate, "Date valeur"), _
	Array(fldMemo, "Libelle"), _
	Array(fldAmount, "Montant"), _
	Array(fldSkip, "Solde") _
)
...
aPropertyList = Array( _
	Array("AXAFrCompteVideOK", "Permettre des numéros compte vides", _
		"Permettre l'utilisation des numéros compte vides. Attention: les fichiers OFX exigent un numéro compte non-vide!", _
		ptBoolean), _
	Array("AXAFrCompteB", "Numéro compte banque", _
		"Le numéro du compte banque pour AXA France.", _
		ptString), _
	Array("AXAFrCompteC", "Numéro compte CC", _
		"Le numéro du compte carte crédit pour AXA France.", _
		ptString) _
	)
...
Function RecogniseTextFile()
	...
	If UBound(vFields) <> NumFieldsExpected Then
		If DebugRecognition Then
			...
			& NumFieldsExpected & " - " & sLine,,ScriptName
		End If
		Exit function
	End If
...
Function LoadTextFile()
	...
	Dim dMinDate	' earliest date in file - if we need an opening balance date
	...
	Do While Not AtEOF()
		sLine = ReadLine()
' Axa France: detect bank account or credit card
		If AccountNum = "" Then
			AccountNum = GetAccountNum(sLine)
			If AccountNum = "" And Not GetProperty("AXAFrCompteVideOK") Then
				Exit Function
			End If
		End If
...
' find the payee, transaction type and txn date if we can
			...
			If Len(TxnDatePattern) > 0 Then
				...
				If TypeName(vDateBits) = "Variant()" Then
					Txn.TxnDate = DateSerial(Year(Stmt.OpeningBalance.BalDate), CInt(vDateBits(2)), CInt(vDateBits(1)))
...
' Axa France: get balance from new format (first line - txns are in reverse order!)
			If vFields(4) <> "Eur" And Stmt.ClosingBalance.BalDate = NODATE Then
				Stmt.ClosingBalance.Amt = ParseNumber(vFields(4), DecimalSeparator)
				Stmt.ClosingBalance.BalDate = Txn.BookDate
			End If
			
' Axa France: find the cheque number
			If StringMatches(Txn.Memo, "CHEQUE N \d+") Then
				Txn.CheckNum = Mid(Txn.Memo, 10)
			End If
' Axa France: derive the payee from the memo
			If Txn.TxnDateValid Then
				Txn.Payee = Mid(Txn.Memo, 7)
			Elseif StartsWith(Txn.Memo, "VIR RECU ") Then
				Txn.Payee = Mid(Txn.Memo, 10)
			Elseif StartsWith(Txn.Memo, "VIR ") Then
				Txn.Payee = Mid(Txn.Memo, 5)
			Elseif StartsWith(Txn.Memo, "PREL ") Then
				Txn.Payee = Mid(Txn.Memo, 6)
			Elseif Not StartsWith(Txn.Memo, "CHEQUE N ") Then
				Txn.Payee = Txn.Memo
			End If
...
Function GetAccountNum(sLine)
	Dim sAcct
	Dim bIsCC:	bIsCC = (Mid(sLine, 12, 1) <> "-")
	Dim sProp, sPrompt
	If bIsCC Then
		sProp = "AxaFrCompteC"
' GetProperty and SetProperty don't work with variables as the name parameter! This bug will be fixed in 3.4, but
' for now we have to avoid that construction
		sAcct = GetProperty("AxaFrCompteC")
		sPrompt = "Veuillez donner le numéro de compte de votre carte crédit."
	Else
		sProp = "AxaFrCompteB"
		sAcct = GetProperty("AxaFrCompteC")
		sPrompt = "Veuillez donner le numéro de votre compte banque."
	End If
	If Len(sAcct) = 0 and Not GetProperty("AXAFrCompteVideOK") Then
		sAcct = InputBox(sPrompt, _
			"AXA - Compte inconnu", _
			sAcct)
		If Len(sAcct) = 0 Then
			Message True, True, "Ne peut pas continuer sans numéro compte!"
			Abort
			Exit Function
		End If
' see comment above!
'		SetProperty sProp, sAcct
		If bIsCC Then
			SetProperty "AxaFrCompteC", sAcct
		Else
			SetProperty "AxaFrCompteB", sAcct
		End If
		SaveProperties ScriptName, aPropertyList
	End If
	GetAccountNum = sAcct
