...
Const ScriptVersion = "$Header: /MT2OFX/NationwideCC-CSV.vbs 5     28/03/11 0:01 Colin $"
...
aPropertyList = Array( _
    ...
    Array("LastAccount", "Last used account number", _
        "Account number used in last conversion", _
        ptString _
    ), _
    Array("MultiAccount", "Multiple accounts", _
        "If you have more than one Nationwide Credit Card", _
        ptBoolean _
    ) _
)
...
' Nationwide: get balance from header line
    ...
    AccountNum = GetAccountNumber("NationwideCCAcct", "MultiAccount", "LastAccount", FormatName, "Select the account number for this statement", "ValidateNW")
...
' download periods can overlap)
			...
			Else
				iTxnSeq = 1
			End If
' 20101211 CS Use default FITID generation - override below produces duplicate FITIDs when txns are out of order
