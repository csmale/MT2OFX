' MT2OFX Input Processing Script ANZ Money Manager CSV format
...
With Params
	...
	.ScriptName = "ANZMoneyManager-CSV"
	.FormatName = "ANZ Money Manager CSV"
	...
	.SkipHeaderLines = 2	' number of lines to skip before the transaction data
...
' to the text between the top-level parentheses.
	...
	.MonthNames = Empty
' Status,Date,Original Description,Split Type,Category,Currency,Amount,User Description,Memo,Classification
	.Fields = Array( _
		Array(fldSkip, "Status"), _
		Array(fldBookDate, "Date"), _
		Array(fldMemo, "Original Description"), _
		Array(fldSkip, "Split Type"), _
		Array(fldCategory, "Category"), _
		Array(fldCurrency, "Currency"), _
		Array(fldAmount, "Amount"), _
		Array(fldMemo, "User Description"), _
		Array(fldMemo, "Memo"), _
		Array(fldSkip, "Classification") _
	)
...
'MsgBox "In header callback: " & sLine
    Dim iTmp, sTmp
' GE Coles Group Source MasterCard (AU) - xxxxxxxxxxxxxxx2212 - Transactions,,,,,,,,,
    iTmp = InStr(sLine, " - ")
    If iTmp > 0 Then
        sTmp = Mid(sLine, iTmp + 3)
        iTmp = InStr(sTmp, " - ")
        If iTmp > 0 Then
            Params.AccountNum = Left(sTmp, iTmp-1)
        End If
    End If
    HeaderCallback = True
End Function
