...
Const ScriptVersion = "$Header: /MT2OFX/HandelsbankenSE-WEB.vbs 1     26/11/08 0:00 Colin $"

Const ScriptName = "HandelsbankenSE-WEB"
Const FormatName = "Handelsbanken Web format"
...
MonthNames = Array("jan","feb","mrt","apr","mei","jun","jul","aug","sep","okt","nov","dec")
...
' MSHTML.HTMLDocument object
Dim xHTML		: Set xHTML=Nothing			' full HTML document
Dim xHTMLTable	: Set xHTMLTable=Nothing	' our table
...
Function FindTable()
	Dim xTmp
	FindTable = False
	If xHTMLTable Is Nothing Then
		If xHTML Is Nothing Then
			Set xHTML = GetHTMLDocument()
			If TypeName(xHTML) <> "HTMLDocument" Then
MsgBox "not HTMLDocument but " & TypeName(xHTML)
				Exit Function
			End If
		End If
		If xHTML Is Nothing Then
			Exit Function
		End If
		Set xTmp = HTMLForm(xHTML, 1)
		If xTmp Is Nothing Then
MsgBox "no form"
			Exit Function
		End If
		Set xHTMLTable = HTMLTable(xTmp, 1)
		If xHTMLTable Is Nothing Then
MsgBox "no table"
			Exit Function
		End If
' we think we have found the transaction table - do a few checks to verify
		Set xTmp = HTMLRow(xHTMLTable, 2)
		If xTmp Is Nothing Then
MsgBox "no header row"
			Exit Function
		End If
		If xTmp.innertext <> "Kaartrekening - overzicht en details" Then
MsgBox "'" & xTmp.innertext & "'"
			Exit Function
		End If
	End If
	FindTable = True
...
MsgBox "cannot find main table"
		Exit Function
	End If

MsgBox "got main table"
	Set xTmp = HTMLTable(xHTMLTable, 2)
	If xTmp Is Nothing Then
		Exit Function
	End If
MsgBox "got subtable"
	Set xTmp = HTMLRow(xTmp, 4)
' should have joining year, stmt dates, card number
	sTmp = HTMLCell(xTmp, 2).innertext
	vFields = Split(sTmp, "-")
	dFromDate = ParseDateEx(Trim(vFields(0)), "MDY", " ")
	dToDate = ParseDateEx(Trim(vFields(1)), "MDY", " ")
	sAcct = HTMLCell(xTmp, 3).innertext
	MsgBox dFromDate & "-" & dToDate & " card no " & sAcct
	MsgBox xTmp.innertext
	Exit Function

' Amex NL
' each transaction is on four rows of 2 cells
' row 1: black line
' row 2: date, amount
' row 3: description (colspan 2)
' row 4: black line
