...
Const ScriptVersion = "$Header: /MT2OFX/NationwideSavings-CSV.vbs 2     30/01/11 13:28 Colin $"

Const ScriptName = "NationwideSavings-CSV"
Const FormatName = "Nationwide eSavings account CSV"
...
'Available balance: ,£1036.05,,,,
	...
	Do While Not AtEOF()
		...
		If Len(sLine) > 0 And Left(sLine,1) <> CSVSeparator Then
			...
			End If
			If UBound(vFields) < MinFieldsExpected Or UBound(vFields) > MaxFieldsExpected Then
				DebugMessage "Wrong number of fields - " & CStr(UBound(vFields)) & " - " & sLine
				Abort
				Exit function
			End If
