' MT2OFX Input Processing Script Credit Suisse CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/CreditSuisseCH-CSV.vbs 2     9/03/08 23:21 Colin $"

Const ScriptName = "CreditSuisseCH-CSV"
Const FormatName = "Credit Suisse CSV Format"
...
Const CurrencyCode = "CHF"	' default if not specified in file
...
' du 16.08.05 à 12:38   H
Const TxnDatePattern = ".*(?:du|of|vom) (\d\d)\.(\d\d)\.(\d\d) (?:à|at|um) (\d\d)\:(\d\d) +H.*"	' pattern to find transaction date in the memo
...
Dim aFields
aFields = Array( _
	Array(fldBookDate, "Date comptable", "Date comptable|Booking Date"), _
	Array(fldMemo, "Texte", "Texte|Text"), _
	Array(fldAmtDebit, "Débit", "Débit|Debit"), _
	Array(fldAmtCredit, "Crédit", "Crédit|Credit"), _
	Array(fldValueDate, "Date de valeur", "Date de valeur|Value Date"), _
	Array(fldClosingBal, "Solde", "Solde|Balance") _
)
...
Function LoadTextFile()
	...
	Dim i
	Dim dBal, dBalDate	' temp balance date
	...
	For i=1 To SkipHeaderLines
		sLine = ReadLine()
		If StartsWith(sLine, "N° de compte,") Then
			sTmp = Mid(sLine, 14)
			sAcct = Left(sTmp, InStr(sTmp, ",")-1)
	...
	Do While Not AtEOF()
		sLine = ReadLine()
		If Len(sLine) > 0 And Left(sLine,1) <> CSVSeparator And _
			(Not StartsWith(sLine, "Total de la colonne,")) And (Not StartsWith(sLine, "Total of Column,")) Then
			If CSVSeparator = "" Then
				vFields = ParseLineFixed(sLine, TxnLinePattern)
			Else
				vFields = ParseLineDelimited(sLine, CSVSeparator)
...
' find the payee, transaction type and txn date if we can
			...
			If Len(TxnDatePattern) > 0 Then
				Txn.TxnDate = TransDate(Txn.Memo)
				Txn.TxnDateValid = (Txn.TxnDate <> NODATE)
			End If
			If StartsWith(sMemo, "Paiement Maestro") Then
				Txn.TxnType = "POS"
				Txn.Payee = Trim(Mid(sMemo, 97, 36))
			ElseIf StartsWith(sMemo, "Maestro payment") Then
				Txn.TxnType = "POS"
				Txn.Payee = Trim(Mid(sMemo, 49, 36))			
			ElseIf StartsWith(sMemo, "Prélèvement") Or StartsWith(sMemo, "Cash Service prélèv") Then
				Txn.Payee = "Prélèvement"
				Txn.TxnType = "ATM"
			ElseIf StartsWith(sMemo, "Recouvrement") Then
				Txn.Payee = Trim(Mid(sMemo, 49, 48))
			ElseIf StartsWith(sMemo, "Ordre permanent") Then
				Txn.TxnType = "REPEATPMT"
				Txn.Payee = Trim(Mid(sMemo, 49, 48))
			ElseIf StartsWith(sMemo, "Bonification") Then
				Txn.Payee = Trim(Mid(sMemo, 49, 48))
			ElseIf StartsWith(sMemo, "Payment") Then
				Txn.Payee = Trim(Mid(sMemo, 9, 48))
			End If
