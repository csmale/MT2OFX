' MT2OFX Input Processing Script Standard Chartered (Ghana) CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/StandardCharteredGH-CSV.vbs 1     21/08/09 22:10 Colin $"
...
With Params
	...
	.ScriptName = "StandardCharteredGH-CSV"
	.FormatName = "Standard Chartered (Ghana) CSV"
	...
	.CurrencyCode = "GHS"	' default if not specified in file
	.ColumnHeadersPresent = True	' are the column headers in the file?
	.SkipHeaderLines = 3	' number of lines to skip before the transaction data
...
' to the text between the top-level parentheses.
	...
	.MonthNames = Empty
' the following is only for the first two lines of the file. thereafter we switch to a different pattern
...
Dim dAvailBal, dOpeningBal, dClosingBal
Dim iSeq, dLast
...
' in the class
'Account Number,Account Title,Account Type,Currency,Available Balance,Ledger Balance
'0100139328300,MARK DAVIES,CURRENT ACCOUNTS,GHS,343.58999999999997498889570124447345733642578125,343.59
...
'MsgBox "In header callback: " & sLine
    ...
    If Len(Trim(sLine)) > 0 Then
        ...
        If IsNumeric(vFields(1)) Then
            Params.AccountNum = vFields(1)
            Params.CurrencyCode = vFields(4)
            dAvailBal = ParseNumber(vFields(5), Params.DecimalSeparator)
        End If
    End If
    HeaderCallback = True
End Function
...
' sort out a transaction ID - date, sequence are provided!
    If dLast <> t.BookDate Then
        iSeq = 1
        dLast = t.BookDate
    Else
        iSeq = iSeq + 1
    End If
    t.FITID = CStr(Year(t.BookDate)) & _
        Right("0" & CStr(Month(t.BookDate)), 2) & _
        Right("0" & CStr(Day(t.BookDate)), 2) & _
        "." & CStr(iSeq)
...
'MsgBox "In statement callback"
    Stmt.AvailableBalance.Amt = dAvailBal
...
Function IsValidTxnLine(sLine)
    IsValidTxnLine = (IsNumeric(Left(sLine, 1)) And Left(sLine,1) <> ",")
',Opening Balance,,, 1288.590
    If StartsWith(sLine, ",Opening Balance,,,") Then
        dOpeningBal = ParseNumber(Mid(sLine, 20), Params.DecimalSeparator)
    ElseIf StartsWith(sLine, ",Closing Balance,,,") Then
        dClosingBal = ParseNumber(Mid(sLine, 20), Params.DecimalSeparator)
    End If
'MsgBox "In IsValidTxnLine callback: " & IsValidTxnLine & ": " & sLine
End Function
