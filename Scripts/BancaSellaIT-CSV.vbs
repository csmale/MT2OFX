' MT2OFX Input Processing Script Banca Sella CSV format

Option Explicit

Const ScriptVersion = "$Header: /MT2OFX/BancaSellaIT-CSV.vbs 3     20/04/08 10:01 Colin $"

Const ScriptName = "BancaSellaIT-CSV"
Const FormatName = "Banca Sella CSV Format"
...
'"Codice Operazione","Data operazione","Data valuta","Causale","Divisa","Importo a debito","Importo a credito",
Dim aFields
aFields = Array( _
	Array(fldFITID, "Codice Operazione"), _
	Array(fldBookDate, "Data operazione"), _
	Array(fldValueDate, "Data valuta"), _
	Array(fldMemo, "Causale"), _
	Array(fldCurrency, "Divisa"), _
	Array(fldAmtDebit, "Importo a debito"), _
	Array(fldAmtCredit, "Importo a credito"), _
	Array(fldSkip, "") _
)
...
Function LoadTextFile()
	...
	If Len(sAcct) = 0 Then sAcct = AccountNum
	If Len(sAcct) = 0 Then
		Message True, True, "Please enter the account number in the script properties through the Options dialog.", FormatName
		Abort
	End If
	...
	Do While Not AtEOF()
		sLine = ReadLine()
		If Len(sLine) > 0 And Left(sLine,1) <> CSVSeparator And Left(sLine, 3) <> """""," Then
			If CSVSeparator = "" Then
				vFields = ParseLineFixed(sLine, TxnLinePattern)
			Else
				vFields = ParseLineDelimited(sLine, CSVSeparator)
...
' find the payee, transaction type and txn date if we can
			sMemo = Txn.Memo
			If StartsWith(sMemo, "PD VE ") Then
				Txn.Payee = Mid(sMemo, 7)
				Txn.TxnType = "DIRECTDEBIT"
			ElseIf StartsWith(sMemo, "CP VE ATM ") Then
				Txn.Payee = Mid(sMemo, 11)
				Txn.TxnType = "ATM"
			ElseIf StartsWith(sMemo, "CO VE ") Or StartsWith(sMemo, "CP VE ") Then
				Txn.Payee = Mid(sMemo, 7)
			End If
...
' keep tabs on the statement/balance Date
			...
			FirstTxn = False
		Else
			If StartsWith(sLine, """"",""Saldo ") Then
'"","Saldo al 3/2/2008 11:23:38 PM","2103.31","",
				vFields = ParseLineDelimited(sLine, CSVSeparator)
				If TypeName(vFields) = "Variant()" Then
'					MsgBox "balance found: " & vFields(3) & " at " & vFields(2)
					Stmt.ClosingBalance.Amt = ParseNumber(vFields(3), DecimalSeparator)
					sTmp = Mid(vFields(2), 10)
					sTmp = Left(sTmp, InStr(vFields(2), " ")-1)
					Stmt.ClosingBalance.BalDate = ParseDateEx(sTmp, "DMY", "/")
				End If
			End If
		End If
	Loop
